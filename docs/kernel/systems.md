...
The handler runs entirely in `kernel/sched/sysinfo.zig::sysSysInfo` (the syscall dispatch layer just forwards to it). The flow must satisfy two independent §4.55 invariants:
