/// I/O APIC emulation for Linux guest boot.
/// Intel 82093AA datasheet (Order Number: 290566-001).
/// The IOAPIC is accessed via indirect register addressing:
///   IOREGSEL at 0xFEC00000 (selects register index)
///   IOWIN at 0xFEC00010 (reads/writes selected register)
/// Section 3.0: Register Description.

const log = @import("log.zig");
const lapic = @import("lapic.zig");

pub const IOAPIC_BASE: u64 = 0xFEC00000;

// Memory-mapped register offsets (Table 1)
const IOREGSEL_OFF: u32 = 0x00;
const IOWIN_OFF: u32 = 0x10;

// IOAPIC register indices (Table 2)
const REG_ID: u8 = 0x00; // IOAPICID — R/W, bits 27:24
const REG_VER: u8 = 0x01; // IOAPICVER — RO, version + max redir
const REG_ARB: u8 = 0x02; // IOAPICARB — RO, arbitration ID
const REG_REDTBL_BASE: u8 = 0x10; // IOREDTBL[0] lo, 0x11 = IOREDTBL[0] hi, etc.

/// Number of redirection table entries (24 IRQ pins).
const NUM_REDIR_ENTRIES: u5 = 24;

// IOAPIC state — file-scope globals.

/// IOREGSEL: currently selected register index.
var ioregsel: u8 = 0;

/// IOAPIC ID register. Only bits 27:24 are meaningful (Section 3.2.1).
var ioapic_id: u32 = 0;

/// 24-entry redirection table, 64 bits each (Section 3.2.4).
/// Reset default: bit 16 (mask) set = all masked (xxx1_xxxx_xxxx_xxxxh).
var redir_table: [NUM_REDIR_ENTRIES]u64 = .{@as(u64, 1) << 16} ** NUM_REDIR_ENTRIES;

/// IRQ line state for level-triggered interrupt tracking.
var irq_level: u32 = 0;

var log_count: u32 = 0;

/// Initialize the IOAPIC to reset state.
pub fn init() void {
    ioregsel = 0;
    ioapic_id = 0;
    irq_level = 0;
    for (&redir_table) |*entry| {
        // Default: masked (bit 16 = 1), all other bits 0
        entry.* = @as(u64, 1) << 16;
    }
    log.print("IOAPIC: init done\n");
}

/// Handle MMIO read at offset from IOAPIC base 0xFEC00000.
/// Only IOREGSEL (offset 0x00) and IOWIN (offset 0x10) are accessible.
pub noinline fn read(offset: u32) u32 {
    return switch (offset) {
        IOREGSEL_OFF => ioregsel,
        IOWIN_OFF => readRegister(ioregsel),
        else => blk: {
            if (log_count < 10) {
                log_count += 1;
                log.print("IOAPIC: bad read offset=0x");
                log.hex32(offset);
                log.print("\n");
            }
            break :blk 0;
        },
    };
}

/// Handle MMIO write at offset from IOAPIC base 0xFEC00000.
pub noinline fn write(offset: u32, value: u32) void {
    switch (offset) {
        IOREGSEL_OFF => ioregsel = @truncate(value & 0xFF),
        IOWIN_OFF => writeRegister(ioregsel, value),
        else => {
            if (log_count < 10) {
                log_count += 1;
                log.print("IOAPIC: bad write offset=0x");
                log.hex32(offset);
                log.print("\n");
            }
        },
    }
}

/// Assert an IRQ line. Routes through the redirection table to the LAPIC.
/// Section 3.2.4: Redirection table entry format.
pub noinline fn assertIrq(irq: u5) void {
    if (irq >= NUM_REDIR_ENTRIES) return;
    const entry = redir_table[irq];

    // Bit 16: mask. If masked, ignore.
    if (entry & (1 << 16) != 0) return;

    const trigger_mode = (entry >> 15) & 1; // 0=edge, 1=level
    const irq_bit = @as(u32, 1) << irq;

    if (trigger_mode == 0) {
        // Edge-triggered: deliver on rising edge (transition from 0 to 1).
        // Section 3.2.4: "A new edge on that Interrupt input pin will not be
        // recognized until the IOAPIC broadcasts the corresponding message."
        if (irq_level & irq_bit != 0) return; // Already asserted, no new edge
        irq_level |= irq_bit;
    } else {
        // Level-triggered: deliver if not already pending (remote IRR = 0).
        // Bit 14: Remote IRR
        if (entry & (1 << 14) != 0) return; // Already accepted, waiting for EOI
        irq_level |= irq_bit;
        // Set Remote IRR (bit 14) to indicate delivery in progress
        redir_table[irq] |= (1 << 14);
    }

    deliverInterrupt(irq, entry);
}

/// De-assert an IRQ line.
pub fn deassertIrq(irq: u5) void {
    if (irq >= NUM_REDIR_ENTRIES) return;
    irq_level &= ~(@as(u32, 1) << irq);
}

/// Handle EOI from LAPIC for a level-triggered interrupt.
/// Clears Remote IRR (bit 14) in the matching redirection table entry.
pub fn handleEOI(vector: u8) void {
    for (&redir_table, 0..) |*entry, i| {
        const entry_vector: u8 = @truncate(entry.* & 0xFF);
        if (entry_vector == vector and (entry.* & (1 << 14) != 0)) {
            // Clear Remote IRR
            entry.* &= ~@as(u64, 1 << 14);
            // If the IRQ line is still asserted (level-sensitive), re-deliver
            if (irq_level & (@as(u32, 1) << @as(u5, @truncate(i))) != 0) {
                entry.* |= (1 << 14);
                deliverInterrupt(@truncate(i), entry.*);
            }
            return;
        }
    }
}

// --- Internal helpers ---

/// Read an IOAPIC register by index (via IOREGSEL).
fn readRegister(index: u8) u32 {
    return switch (index) {
        REG_ID => ioapic_id,
        // Section 3.2.2: Version=0x11, max redir entry=23 (0x17)
        REG_VER => 0x00170011,
        // Section 3.2.3: Arbitration ID (same as IOAPIC ID)
        REG_ARB => ioapic_id,
        // Redirection table: 0x10-0x3F
        REG_REDTBL_BASE...0x3F => blk: {
            const reg_off = index - REG_REDTBL_BASE;
            const entry_idx = reg_off / 2;
            if (entry_idx >= NUM_REDIR_ENTRIES) break :blk 0;
            const entry = redir_table[entry_idx];
            if (reg_off & 1 == 0) {
                // Low 32 bits
                break :blk @truncate(entry);
            } else {
                // High 32 bits
                break :blk @truncate(entry >> 32);
            }
        },
        else => 0,
    };
}

/// Write an IOAPIC register by index (via IOREGSEL).
fn writeRegister(index: u8, value: u32) void {
    switch (index) {
        REG_ID => ioapic_id = value & 0x0F000000, // Only bits 27:24 writable
        REG_VER, REG_ARB => {}, // Read-only registers, writes ignored
        REG_REDTBL_BASE...0x3F => {
            const reg_off = index - REG_REDTBL_BASE;
            const entry_idx = reg_off / 2;
            if (entry_idx >= NUM_REDIR_ENTRIES) return;
            // Bits 12 (delivery status) and 14 (remote IRR) are read-only
            const ro_mask_lo: u64 = (1 << 12) | (1 << 14);
            if (reg_off & 1 == 0) {
                // Low 32 bits: preserve RO bits, write others
                const old = redir_table[entry_idx];
                const preserved = old & (ro_mask_lo | 0xFFFFFFFF_00000000);
                const written = @as(u64, value) & ~ro_mask_lo;
                redir_table[entry_idx] = preserved | written;
            } else {
                // High 32 bits (destination field)
                const old = redir_table[entry_idx];
                redir_table[entry_idx] = (old & 0x00000000_FFFFFFFF) | (@as(u64, value) << 32);
            }
        },
        else => {},
    }
}

/// Deliver an interrupt from a redirection table entry to the LAPIC.
fn deliverInterrupt(irq: u5, entry: u64) void {
    _ = irq;
    const vector: u8 = @truncate(entry & 0xFF);
    const delivery_mode: u3 = @truncate((entry >> 8) & 0x7);

    switch (delivery_mode) {
        0b000, 0b001 => {
            // Fixed / Lowest Priority — deliver vector to LAPIC.
            // For single vCPU, lowest priority == fixed.
            if (vector < 16) return; // Illegal vector, ignore
            lapic.injectExternal(vector);
        },
        0b010 => {
            // SMI — stubbed for now
            if (log_count < 10) {
                log_count += 1;
                log.print("IOAPIC: SMI delivery (stubbed)\n");
            }
        },
        0b100 => {
            // NMI — stubbed for now
            if (log_count < 10) {
                log_count += 1;
                log.print("IOAPIC: NMI delivery (stubbed)\n");
            }
        },
        0b101 => {
            // INIT — stubbed for now
            if (log_count < 10) {
                log_count += 1;
                log.print("IOAPIC: INIT delivery (stubbed)\n");
            }
        },
        0b111 => {
            // ExtINT — deliver as external interrupt
            lapic.injectExternal(vector);
        },
        else => {}, // Reserved modes, ignore
    }
}
