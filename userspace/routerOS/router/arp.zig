...
pub fn sendRequest(ctx: *RouterContext, iface: Interface, target_ip: [4]u8) void {
    var pkt: [60]u8 = undefined;
    @memset(&pkt, 0);
    @memset(pkt[0..6], 0xFF);
    @memcpy(pkt[6..12], ctx.ifaceMac(iface));
    pkt[12] = 0x08;
    pkt[13] = 0x06;
    pkt[14] = 0x00;
    pkt[15] = 0x01;
    ...
    pkt[20] = 0x00;
    pkt[21] = 0x01;
    @memcpy(pkt[22..28], ctx.ifaceMac(iface));
    @memcpy(pkt[28..32], ctx.ifaceIp(iface));
    @memset(pkt[32..38], 0);
    @memcpy(pkt[38..42], &target_ip);
    _ = ctx.ifaceChan(iface).send(&pkt);
}

pub fn handle(ctx: *RouterContext, iface: Interface, pkt: []u8, len: u32) ?[]u8 {
    ...
    if (opcode == 0x0001 and !util.eql(pkt[arp_start + 24 ..][0..4], my_ip)) return null;
