...
pub fn main(perm_view_addr: u64) void {
    syscall.write("ntp_client: started\n");
    ...
    syscall.write("ntp_client: connected to router\n");
            ...
            e.field0 > shm_protocol.COMMAND_SHM_SIZE)
        {
            ...
            if (vm.val >= 0) {
                if (syscall.shm_map(e.handle, @intCast(vm.val), 0) == 0) {
                    ...
                    syscall.write("ntp_client: console channel connected\n");
                }
            }
            break;
        }
...
fn sendNtpRequest() void {
    ...
    sync_pending = true;
    send_time_ns = now();
    syscall.write("ntp_client: NTP request sent\n");
}
...
fn handleNtpResponse(payload: []const u8) void {
    ...
    syscall.write("ntp_client: synced\n");
