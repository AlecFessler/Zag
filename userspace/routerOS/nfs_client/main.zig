...
pub fn main(perm_view_addr: u64) void {
    syscall.write("nfs_client: started\n");
    ...
    syscall.write("nfs_client: connected to router\n");
    ...
    has_router = true;
    syscall.write("nfs_client: router channel open\n");
    ...
    syscall.write("nfs_client: bound UDP port\n");
            ...
            e.field0 > shm_protocol.COMMAND_SHM_SIZE)
        {
            ...
            if (vm.val >= 0) {
                if (syscall.shm_map(e.handle, @intCast(vm.val), 0) == 0) {
                    ...
                    syscall.write("nfs_client: console channel connected\n");
                }
            }
            break;
        }
...
fn sendMountRequest() void {
    ...
    state = .mount_pending;
    syscall.write("nfs_client: MOUNT request sent\n");
}
    ...
    switch (state) {
        .mount_pending => {
            if (nfs3.parseMountReply(payload, pending_xid)) |fh| {
                ...
                mounted = true;
                state = .mounted;
                syscall.write("nfs_client: mounted ");
                syscall.write(EXPORT_PATH);
                syscall.write("\n");
