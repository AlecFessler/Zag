...
fn testZombieChain() void {
    ...
    for (0..child_exit_elf.len) |i| { dest[i] = child_exit_elf[i]; }

    const signal_ptr: *volatile u64 = @ptrFromInt(vm_result.val2 + aligned_size - 8);
    signal_ptr.* = 0;

    const child_rights = (perms.ProcessRights{
        .grant_to = true, .spawn_thread = true, .spawn_process = true,
        .mem_reserve = true, .shm_create = true,
    }).bits();
    const proc_handle = syscall.proc_create(@intFromPtr(spawner_elf.ptr), spawner_elf.len, child_rights);
    if (proc_handle <= 0) { t.fail("proc_create for spawner failed"); return; }

    const grant_rights = (perms.SharedMemoryRights{
        .read = true, .write = true, .execute = true, .grant = true,
    }).bits();
