...
pub const PhysicalMemoryManager = struct {
    ...
    pub fn init(backing_allocator: *Allocator) PhysicalMemoryManager {
        return .{
            .backing_allocator = backing_allocator,
        };
    }

    pub fn allocator(self: *PhysicalMemoryManager) Allocator {
        return .{
            .ctx = self,
            .vtable = &.{
                .alloc = alloc,
    ...
    fn alloc(ctx: *anyopaque, n: usize, alignment: usize) AllocationError![*]u8 {
        const self: *PhysicalMemoryManager = @alignCast(@ptrCast(ctx));
