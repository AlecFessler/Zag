...
pub const BitmapFreeList = struct {
    base_addr: usize,
    block_size: usize,
    hint: usize,
    bitmap: []Word,
    ...
    pub fn setBit(self: *BitmapFreeList, addr: usize, val: u1) void {
...
test "getNextFree returns expected free blocks in order" {
    ...
    var freelist = try BitmapFreeList.init(0x1000, 0x1000, 5, true, &allocator);
...
test "extra bits in bitmap word are zeroed on init" {
    ...
    var freelist = try BitmapFreeList.init(0x0, 0x1000, num_bits, true, &allocator);
    defer freelist.deinit();
...
test "setBit clears a bit and advances hint when needed" {
    var allocator = std.testing.allocator;
    var freelist = try BitmapFreeList.init(0x1000, 0x1000, 3, false, &allocator);
    defer freelist.deinit();
