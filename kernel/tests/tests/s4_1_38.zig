...
/// §4.1.38 — `ThreadHandleRights.pmu` gates whether the caller may operate on a specific thread's PMU state; it is required in addition to `ProcessRights.pmu` for every PMU syscall that takes a thread handle.
pub fn main(_: u64) void {
    ...
    if (start_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.38 pmu_start", syscall.E_PERM, start_rc);
        syscall.shutdown();
    }
    if (read_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.38 pmu_read", syscall.E_PERM, read_rc);
        syscall.shutdown();
    }
    if (reset_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.38 pmu_reset", syscall.E_PERM, reset_rc);
        syscall.shutdown();
    }
    if (stop_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.38 pmu_stop", syscall.E_PERM, stop_rc);
        syscall.shutdown();
    }
