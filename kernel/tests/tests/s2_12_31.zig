...
pub fn main(pv: u64) void {
    ...
    if (thread_handle == 0) {
        t.fail("§2.12.31 no thread handle");
        syscall.shutdown();
    }
    ...
    if (rc != 0) {
        t.failWithVal("§2.12.31 exclude_permanent rc", 0, rc);
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, thread_handle)) |e| {
        if (e.threadExcludePermanent() and !e.threadExcludeOneshot()) {
            ...
            t.fail("§2.12.31 wrong flag state after exclude_permanent");
        }
    } else {
        t.fail("§2.12.31 thread entry vanished");
    }
    syscall.shutdown();
}
