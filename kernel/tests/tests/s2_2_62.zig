...
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§2.2.62 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (thread_handle == 0) {
        t.fail("§2.2.62 no thread handle found");
        syscall.shutdown();
    }
    ...
    if (child_slot == 0xFFFF) {
        t.fail("§2.2.62 child slot not found");
        syscall.shutdown();
    }

    const kill_ret = syscall.thread_kill(thread_handle);
    t.expectEqual("§2.2.62 kill last thread", E_OK, kill_ret);
    ...
    while (attempts < 100000) : (attempts += 1) {
        if (view[child_slot].entry_type == perm_view.ENTRY_TYPE_DEAD_PROCESS) {
            t.pass("§2.2.62 process exited (DEAD_PROCESS)");
            syscall.shutdown();
        }
        syscall.thread_yield();
    }

    t.fail("§2.2.62 entry did not become DEAD_PROCESS");
    syscall.shutdown();
}
