...
/// §4.1.2 — `ProcessHandleRights` bit 6 is `fault_handler`.
...
/// fault_handler back to us (P1 = root). Because §4.1.2 is enforced by
...
pub fn main(pv: u64) void {
    ...
    if (syscall.ipc_call(child_handle, &.{}, &reply) != 0) {
        t.fail("§4.1.2 first ipc_call");
        syscall.shutdown();
    }
    const pre_child_rights = reply.words[0];
    const fh_bit_proc: u64 = 0x80;
    if ((pre_child_rights & fh_bit_proc) == 0) {
        t.failWithVal("§4.1.2 pre-transfer child fh bit", @bitCast(fh_bit_proc), @bitCast(pre_child_rights));
        syscall.shutdown();
    }
    ...
    if (p1_fh_count != 1) {
        t.failWithVal("§4.1.2 P1 fault_handler entry count", 1, @bitCast(@as(u64, p1_fh_count)));
        syscall.shutdown();
    }
    ...
    if (syscall.ipc_call(child_handle, &.{}, &reply) != 0) {
        t.fail("§4.1.2 second ipc_call");
        syscall.shutdown();
    }
    const post_child_rights = reply.words[0];
    if ((post_child_rights & fh_bit_proc) != 0) {
        t.failWithVal("§4.1.2 post-transfer child fh bit not cleared", 0, @bitCast(post_child_rights));
        syscall.shutdown();
    }
