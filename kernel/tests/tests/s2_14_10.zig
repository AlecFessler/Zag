...
pub fn main(_: u64) void {
    ...
    if (a_i <= 0) {
        t.fail("§2.14.10 thread_create A");
        syscall.shutdown();
    }
    ...
    if (b_i <= 0) {
        t.fail("§2.14.10 thread_create B");
        _ = syscall.thread_kill(a_h);
        syscall.shutdown();
    }
    ...
    if (syscall.pmu_read(a_h, @intFromPtr(&s_cont)) != syscall.E_OK) {
        t.fail("§2.14.10 pmu_read contended");
    ...
    if (ok_nonzero and within_band) {
        ...
        t.failWithVal("§2.14.10 counters not preserved", @bitCast(c_solo), @bitCast(c_cont_delta));
    }
