...
pub fn main(pv: u64) void {
    ...
    if (h_ret < 0) {
        t.fail("§2.10.2 thread_create failed");
        syscall.shutdown();
    }
    ...
    if (still_exists) {
        t.fail("§2.10.2 pin entry persisted after revoke");
        syscall.shutdown();
    }
    ...
    if (@atomicLoad(u64, &helper_counter, .acquire) == 0) {
        t.fail("§2.10.2 second thread could not run on unpinned core");
        syscall.shutdown();
    }
