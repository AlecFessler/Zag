...
/// §3.3.32 — `send` without `send_words` right returns `E_PERM`.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);
    // child_send_self_no_words: gives us a handle WITHOUT send_words right
    ...
    // Send via handle without send_words → E_PERM
    const rc = syscall.ipc_send(no_words_handle, &.{0x42});
    t.expectEqual("§3.3.32", E_PERM, rc);
    syscall.shutdown();
}
