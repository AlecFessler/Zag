...
pub fn main(_: u64) void {
    ...
    if (result.val < 0) {
        t.failWithVal("§2.15.3 mem_reserve", 0, result.val);
        syscall.shutdown();
    }
    ...
    if (after.mem_free >= before.mem_free) {
        t.failWithVal(
            "§2.15.3 mem_free did not decrease after commit",
    ...
    if (delta < LARGE_PAGES / 2) {
        t.failWithVal(
            "§2.15.3 mem_free delta too small",
