...
/// §2.5.16 — `irq_ack` atomically clears bit 16 of the device's `field0` in the user permissions view via physmap.
pub fn main(pv: u64) void {
    ...
    for (0..128) |i| {
        ...
        if (rights.irq) {
            ...
            if (rc == syscall.E_OK or rc == syscall.E_INVAL) {
                ...
                t.expectEqual("§2.5.16", 0, @as(i64, @intCast(irq_pending)));
                syscall.shutdown();
            }
        }
    }
    ...
    t.expectEqual("§2.5.16", syscall.E_BADHANDLE, rc);
    syscall.shutdown();
}
