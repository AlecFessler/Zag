...
pub fn main(pv: u64) void {
    ...
    if (child_thread == 0) {
        t.fail("§2.2.25 no child thread handle");
        syscall.shutdown();
    }

    // Kill the child's only thread. Per §2.2.25 this must trigger process
    // death (the child has no restart context).
    const rc = syscall.thread_kill(child_thread);
    if (rc != 0) {
        t.failWithVal("§2.2.25 thread_kill", 0, rc);
        syscall.shutdown();
    }
    ...
    t.fail("§2.2.25 child never became dead_process");
    syscall.shutdown();
}
