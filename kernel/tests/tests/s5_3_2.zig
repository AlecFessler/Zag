...
/// §5.3.2 — `SysInfo.mem_total` is the total physical page count managed by the kernel.
///
...
pub fn main(_: u64) void {
    ...
    if (rc_a != syscall.E_OK) {
        t.failWithVal("§5.3.2 first call", syscall.E_OK, rc_a);
        syscall.shutdown();
    }
    ...
    if (rc_b != syscall.E_OK) {
        t.failWithVal("§5.3.2 second call", syscall.E_OK, rc_b);
        syscall.shutdown();
    }

    if (a.mem_total == 0) {
        t.fail("§5.3.2 mem_total == 0");
        syscall.shutdown();
    }
    if (a.mem_total < 8) {
        t.failWithVal("§5.3.2 mem_total implausibly small", 8, @intCast(a.mem_total));
        syscall.shutdown();
    }
    if (a.mem_free > a.mem_total) {
        t.failWithVal(
            "§5.3.2 mem_free > mem_total",
            @intCast(a.mem_total),
            @intCast(a.mem_free),
        );
        syscall.shutdown();
