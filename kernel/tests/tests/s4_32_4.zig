...
/// §4.32.4 — `thread_kill` on a thread in `.faulted` state returns `E_BUSY`
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.32.4 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.32.4 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }
