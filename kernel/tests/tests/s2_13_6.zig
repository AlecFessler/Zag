...
pub fn main(pv: u64) void {
    ...
    if (cr != syscall.E_OK) {
        t.failWithVal("§2.13.6 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§2.13.6 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§2.13.6 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§2.13.6 set_state", syscall.E_OK, sr);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (exit_token <= 0) {
        t.failWithVal("§2.13.6 recv", 1, exit_token);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (token_u64 == msg_handle) {
        ...
        t.fail("§2.13.6 token != msg.thread_handle");
    }
