...
pub fn main(pv: u64) void {
    ...
    if (cr < 0) {
        t.failWithVal("§2.13.13 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§2.13.13 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§2.13.13 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§2.13.13 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit_token <= 0) {
        t.failWithVal("§2.13.13 recv", 1, exit_token);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (guest_rax != 0x42) {
        t.failWithVal("§2.13.13 rax", 0x42, @as(i64, @bitCast(guest_rax)));
        passed = false;
    }

    if (guest_rip < 5) {
        t.failWithVal("§2.13.13 rip", 5, @as(i64, @bitCast(guest_rip)));
        passed = false;
    }
