...
pub fn main(pv: u64) void {
    ...
    if (thread_handle == 0) {
        t.fail("§2.1.78 no thread handle found");
        syscall.shutdown();
    }
    ...
    if (rc1 != 0) {
        t.failWithVal("§2.1.78 EXCLUDE_NEXT rc", 0, rc1);
        syscall.shutdown();
    }
    if (findThreadEntry(view, thread_handle)) |e| {
        if (!(e.threadExcludeOneshot() and !e.threadExcludePermanent())) {
            t.fail("§2.1.78 EXCLUDE_NEXT: expected bit0=1, bit1=0");
            syscall.shutdown();
        }
    } else {
        t.fail("§2.1.78 thread entry vanished");
        syscall.shutdown();
    }
    ...
    if (rc2 != 0) {
        t.failWithVal("§2.1.78 EXCLUDE_PERMANENT rc", 0, rc2);
        syscall.shutdown();
    }
    if (findThreadEntry(view, thread_handle)) |e| {
        if (!(!e.threadExcludeOneshot() and e.threadExcludePermanent())) {
            t.fail("§2.1.78 EXCLUDE_PERMANENT: expected bit0=0, bit1=1");
            syscall.shutdown();
        }
    } else {
        t.fail("§2.1.78 thread entry vanished");
    ...
    if (rc3 != 0) {
        t.failWithVal("§2.1.78 STOP_ALL rc", 0, rc3);
        syscall.shutdown();
    }
    if (findThreadEntry(view, thread_handle)) |e| {
        if (e.threadExcludeOneshot() or e.threadExcludePermanent()) {
            t.fail("§2.1.78 STOP_ALL: expected both bits clear");
            syscall.shutdown();
        }
    } else {
        t.fail("§2.1.78 thread entry vanished");
