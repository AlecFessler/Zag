...
/// §5.3.10 — `sys_info` with `cores_ptr` null writes only `SysInfo`; no per-core data is written and no scheduler accounting counters are reset.
///
...
pub fn main(_: u64) void {
    ...
    if (syscall.sys_info(@intFromPtr(&info), 0) != syscall.E_OK) {
        t.fail("§5.3.10 null-cores sys_info");
        syscall.shutdown();
    }
    var i: u64 = 0;
    while (i < info.core_count) : (i += 1) {
        ...
        if (c.idle_ns != sentinel or c.busy_ns != sentinel or c.freq_hz != sentinel) {
            t.failWithVal("§5.3.10 cores buffer was written", @intCast(i), 0);
            syscall.shutdown();
        }
    }
    ...
    if (syscall.sys_info(@intFromPtr(&info), 0) != syscall.E_OK) {
        t.fail("§5.3.10 null-cores sys_info (mid-test)");
        syscall.shutdown();
    }
    ...
    if (syscall.sys_info(@intFromPtr(&info), @intFromPtr(&cores)) != syscall.E_OK) {
        t.fail("§5.3.10 post-null sys_info");
        syscall.shutdown();
    }
    var total: u64 = 0;
    var k: u64 = 0;
    ...
    if (total < MIN_TOTAL_NS) {
        t.failWithVal(
            "§5.3.10 null call appears to have reset accounting",
