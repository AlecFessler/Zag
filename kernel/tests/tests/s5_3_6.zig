...
pub fn main(_: u64) void {
    ...
    while (i < info.core_count) : (i += 1) {
        if (cores[i].c_state != 0) {
            t.failWithVal(
                "§5.3.6 c_state non-zero (kernel stub should always report 0)",
                @intCast(i),
    ...
    t.pass("§5.3.6 (c_state is currently a stub that always returns 0 (spec-compliant 'active'); this test will need revisiting when the kernel wires up MSR_CORE_C{1,3,6,7}_RES)");
    syscall.shutdown();
}
