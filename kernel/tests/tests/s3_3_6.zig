...
/// §3.3.6 — `call` with no receiver waiting queues the caller in the target's FIFO wait queue.
///
...
pub fn main(_: u64) void {
    ...
    if (!observed_pending) {
        t.fail("§3.3.6 call completed before any observable wait");
        syscall.shutdown();
    }
    ...
    if (call_result == 0 and reply_val == 0x43) {
        ...
        t.failWithVal("§3.3.6 call rc", 0, call_result);
    }
    syscall.shutdown();
}
