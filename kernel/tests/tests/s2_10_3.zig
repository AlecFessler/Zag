...
/// §2.10.3 — The core_pin handle is a revocation token only; it carries no rights bits (rights = 0).
pub fn main(pv_addr: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§2.10.3 pin", 1, ret);
        syscall.shutdown();
    }
    ...
    for (0..128) |i| {
        ...
        if (e.entry_type == perm_view.ENTRY_TYPE_CORE_PIN and e.handle == handle) {
            if (e.rights == 0) {
                t.pass("§2.10.3 core_pin rights=0");
            } else {
                t.failWithVal("§2.10.3 core_pin rights", 0, @as(i64, e.rights));
            }
            found = true;
            break;
        }
    ...
    if (!found) {
        t.fail("§2.10.3 core_pin entry not found");
    }
