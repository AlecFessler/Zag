...
pub fn main(_: u64) void {
    ...
    if (wret < 0) {
        t.failWithVal("§2.12.15 thread_create", 0, wret);
        syscall.shutdown();
    }
    ...
    while (i < 1000) : (i += 1) {
        if (@atomicLoad(u64, &worker_done, .seq_cst) != 0) {
            t.fail("§2.12.15 worker unblocked with no fault pending");
            syscall.shutdown();
        }
        syscall.thread_yield();
    }
    ...
    if (@atomicLoad(u64, &worker_done, .seq_cst) == 0) {
        t.fail("§2.12.15 worker still blocked after fault arranged");
        syscall.shutdown();
    }
    if (worker_token <= 0) {
        t.failWithVal("§2.12.15 worker token", 1, worker_token);
        syscall.shutdown();
    }
