...
/// §4.1.66 — `fault_read_mem` with `len` = 0 returns `E_INVAL`.
///
/// Must use a proc_handle that has `fault_handler` so the earlier rights
/// check doesn't reject the call first — use the same spawn + transfer
/// pattern as the rest of §4.35.x so `len == 0` is the only failure path.
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.66 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.1.66 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }

    const proc_handle = findFaultHandlerProcHandle(view);
    if (proc_handle == 0) {
        t.fail("§4.1.66 no fault_handler proc handle");
        syscall.shutdown();
    }

    var buf: [8]u8 = undefined;
    const ret = syscall.fault_read_mem(proc_handle, fault_msg.rip, @intFromPtr(&buf), 0);
    t.expectEqual("§4.1.66", E_INVAL, ret);
    syscall.shutdown();
}
