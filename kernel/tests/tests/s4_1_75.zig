...
/// §4.1.75 — `fault_set_thread_mode` requires that the calling process holds `fault_handler` for the owning process of the target thread; returns `E_PERM` otherwise
pub fn main(_: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§4.1.75 proc_create", 1, ret);
        syscall.shutdown();
    }
    ...
    if (call_rc != 0) {
        t.failWithVal("§4.1.75 ipc_call", 0, call_rc);
        syscall.shutdown();
    }
    ...
    if (child_rc == E_PERM) {
        ...
        t.failWithVal("§4.1.75", E_PERM, child_rc);
    }
    syscall.shutdown();
}
