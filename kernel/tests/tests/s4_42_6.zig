/// §4.42.6 — `vm_reply` with `resume_guest` applies modified guest state, including GPR changes, before resuming execution.
...
pub fn main(pv: u64) void {
    ...
    if (cr < 0) {
        t.failWithVal("§4.42.6 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§4.42.6 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.42.6 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.42.6 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit1 <= 0) {
        t.failWithVal("§4.42.6 recv1", 1, exit1);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (gr != syscall.E_OK) {
        t.failWithVal("§4.42.6 get_state", syscall.E_OK, gr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (rr != syscall.E_OK) {
        t.failWithVal("§4.42.6 reply", syscall.E_OK, rr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit2 <= 0) {
        t.failWithVal("§4.42.6 recv2", 1, exit2);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit_tag != EXIT_TAG_IO) {
        t.failWithVal("§4.42.6 exit_tag", EXIT_TAG_IO, @as(i64, exit_tag));
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    var passed = true;
    if (io_port != 0x80) {
        t.failWithVal("§4.42.6 port", 0x80, @as(i64, io_port));
        passed = false;
    }
    ...
    if (io_value != 0xBEEF) {
        t.failWithVal("§4.42.6 value", 0xBEEF, @as(i64, @bitCast(@as(u64, io_value))));
        passed = false;
    }
