...
/// §4.1.53 — `fault_recv` with blocking flag set blocks when the fault box is empty.
///
...
pub fn main(_: u64) void {
    ...
    if (sib_rc < 0) {
        t.failWithVal("§4.1.53 thread_create", 0, sib_rc);
        syscall.shutdown();
    }
    ...
    if (token < 0) {
        t.failWithVal("§4.1.53 fault_recv", 0, token);
        syscall.shutdown();
    }
