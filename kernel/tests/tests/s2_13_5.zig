...
pub fn main(pv: u64) void {
    ...
    if (cr != syscall.E_OK) {
        t.failWithVal("§2.13.5 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (exit_token <= 0) {
        t.failWithVal("§2.13.5 recv", 1, exit_token);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (msg_handle != vcpu_handle) {
        if (msg_handle == 0) {
            t.fail("§2.13.5 buffer not written");
        } else {
            t.failWithVal("§2.13.5 handle mismatch", @bitCast(vcpu_handle), @as(i64, @bitCast(msg_handle)));
        }
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (exit_tag == EXIT_TAG_HLT) {
        ...
        t.failWithVal("§2.13.5 exit_tag", EXIT_TAG_HLT, @as(i64, exit_tag));
    }
