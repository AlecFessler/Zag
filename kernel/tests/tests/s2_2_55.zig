...
/// §2.2.55 — `thread_resume` requires the `resume` right on `thread_handle`; returns `E_PERM` without it.
pub fn main(_: u64) void {
    ...
    if (ret != 0) {
        t.failWithVal("§2.2.55 ipc_call", 0, ret);
        syscall.shutdown();
    }
    const action_rc: i64 = @bitCast(reply.words[3]);
    t.expectEqual("§2.2.55", E_PERM, action_rc);
    syscall.shutdown();
}
