...
pub fn main(pv: u64) void {
        ...
        got_r8 == TEST_R8)
    {
        ...
        if (got_rax != TEST_RAX) {
            t.failWithVal("§4.44.4 rax", @bitCast(TEST_RAX), @bitCast(got_rax));
        } else if (got_rbx != TEST_RBX) {
            t.failWithVal("§4.44.4 rbx", @bitCast(TEST_RBX), @bitCast(got_rbx));
        } else if (got_rcx != TEST_RCX) {
            t.failWithVal("§4.44.4 rcx", @bitCast(TEST_RCX), @bitCast(got_rcx));
        } else if (got_rdx != TEST_RDX) {
            t.failWithVal("§4.44.4 rdx", @bitCast(TEST_RDX), @bitCast(got_rdx));
        } else if (got_rsi != TEST_RSI) {
            t.failWithVal("§4.44.4 rsi", @bitCast(TEST_RSI), @bitCast(got_rsi));
        } else if (got_rdi != TEST_RDI) {
            t.failWithVal("§4.44.4 rdi", @bitCast(TEST_RDI), @bitCast(got_rdi));
        } else if (got_rbp != TEST_RBP) {
            t.failWithVal("§4.44.4 rbp", @bitCast(TEST_RBP), @bitCast(got_rbp));
        } else {
            t.failWithVal("§4.44.4 r8", @bitCast(TEST_R8), @bitCast(got_r8));
        }
    }
