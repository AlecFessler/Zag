...
/// §2.5.9 — Virtual BAR access that begins at or extends beyond `port_count` kills the process with `invalid_read` or `invalid_write`.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    _ = t.requirePioDevice(view, "§2.5.9");
    const dev_handle = findPioDevice(view);
    if (dev_handle == 0) {
        t.fail("§2.5.9 no_pio_device");
        syscall.shutdown();
    }
    ...
    if (child_ret < 0) {
        t.failWithVal("§2.5.9 proc_create", 0, child_ret);
        syscall.shutdown();
    }
    ...
    if (reason == .invalid_read or reason == .invalid_write) {
        ...
        t.failWithVal("§2.5.9", @intFromEnum(perm_view.CrashReason.invalid_read), @intFromEnum(reason));
    }
    syscall.shutdown();
}
