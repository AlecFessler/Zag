...
/// §2.2.58 — `thread_kill` returns `E_OK` on success.
///
...
pub fn main(pv: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§2.2.58 thread_create", 1, ret);
        syscall.shutdown();
    }
    ...
    if (slot == 0xFFFF) {
        t.fail("§2.2.58 thread entry not found pre-kill");
        syscall.shutdown();
    }
    ...
    t.expectEqual("§2.2.58 kill rc", E_OK, kill_ret);
    ...
    while (attempts < 10000) : (attempts += 1) {
        ...
        if (et != perm_view.ENTRY_TYPE_THREAD or h != handle) {
            t.pass("§2.2.58 thread entry removed");
            syscall.shutdown();
        }
        syscall.thread_yield();
    }
    t.fail("§2.2.58 thread entry persisted after kill");
    syscall.shutdown();
}
