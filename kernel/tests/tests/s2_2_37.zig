...
/// §2.2.37 — Fault on the overflow guard (above stack) kills with fault reason `stack_underflow` (§3).
pub fn main(pv: u64) void {
    ...
    t.expectEqual("§2.2.37", @intFromEnum(perm_view.CrashReason.stack_underflow), @as(i64, @intFromEnum(view[slot].processCrashReason())));
    syscall.shutdown();
}
