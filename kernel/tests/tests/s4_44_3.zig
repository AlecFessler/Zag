/// §4.44.3 — `vcpu_get_state` with `guest_state_ptr` not pointing to a writable region of `sizeof(arch.GuestState)` bytes returns `E_BADADDR`.
...
pub fn main(pv: u64) void {
    ...
    const result = syscall.vcpu_get_state(vcpu_handle, 0);
