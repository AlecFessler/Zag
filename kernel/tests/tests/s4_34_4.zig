...
/// §4.34.4 — `fault_reply` with `FAULT_RESUME_MODIFIED` and an unreadable or insufficiently sized `modified_regs_ptr` returns `E_BADADDR`
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.34.4 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (token_ret < 0) {
        t.failWithVal("§4.34.4 fault_recv", 0, token_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.34.4", E_BADADDR, reply_ret);
