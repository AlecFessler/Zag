/// §4.49.3 — `ioapic_deassert_irq` with `irq_num` greater than or equal to 24 returns `E_INVAL`.
...
pub fn main(_: u64) void {
    ...
    const r24 = syscall.ioapic_deassert_irq(24);
    if (r24 != syscall.E_INVAL) {
        t.failWithVal("§4.49.3 irq=24", syscall.E_INVAL, r24);
        passed = false;
    }

    const r99 = syscall.ioapic_deassert_irq(99);
