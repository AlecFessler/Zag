...
/// §2.1.74 — ELF segments and user stacks are placed in the ASLR zone `[0x0000_0000_0000_1000, 0x0000_1000_0000_0000)` with a randomized base.
pub fn main(_: u64) void {
    ...
    for (0..N) |i| {
        ...
        if (rc != 0) {
            t.failWithVal("§2.1.74 ipc_call", 0, rc);
            syscall.shutdown();
        }
    ...
    for (addrs) |a| {
        if (a < ASLR_ZONE_START or a >= ASLR_ZONE_END) {
            t.fail("§2.1.74 address outside ASLR zone");
            syscall.shutdown();
        }
    }
    ...
    if (any_differ) {
        ...
        t.fail("§2.1.74 all children loaded at identical address");
    }
    syscall.shutdown();
}
