...
/// §2.2.9 — `thread_create` inserts a thread handle into the calling process's permissions table with full `ThreadHandleRights` and returns the handle ID (positive u64) on success
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const ret = syscall.thread_create(&worker, 0, 4);
    if (ret <= 0) {
        t.failWithVal("§2.2.9 thread_create", 1, ret);
        syscall.shutdown();
    }
    ...
    if (found and correct_type and correct_rights) {
        ...
        t.fail("§2.2.9 handle not found in perm view");
    } else if (!correct_type) {
        t.fail("§2.2.9 wrong entry type");
    } else {
        t.fail("§2.2.9 wrong rights");
    }
    syscall.shutdown();
}
