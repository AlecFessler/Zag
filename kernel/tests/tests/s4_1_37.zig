...
/// §4.1.37 — `ProcessRights.pmu` gates whether a process may call any PMU syscall that operates on thread state; without it, `pmu_start`, `pmu_read`, `pmu_reset`, and `pmu_stop` return `E_PERM`.
pub fn main(_: u64) void {
    ...
    if (start_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.37 pmu_start", syscall.E_PERM, start_rc);
        syscall.shutdown();
    }
    if (read_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.37 pmu_read", syscall.E_PERM, read_rc);
        syscall.shutdown();
    }
    if (reset_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.37 pmu_reset", syscall.E_PERM, reset_rc);
        syscall.shutdown();
    }
    if (stop_rc != syscall.E_PERM) {
        t.failWithVal("§4.1.37 pmu_stop", syscall.E_PERM, stop_rc);
        syscall.shutdown();
    }
