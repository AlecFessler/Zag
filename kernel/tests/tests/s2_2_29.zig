...
/// §2.2.29 — A `.suspended` thread is not scheduled and does not appear on any run queue.
///
/// Test:
...
pub fn main(pv: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§2.2.29 thread_create", 1, ret);
        syscall.shutdown();
    }
    ...
    if (running_count == 0) {
        t.fail("§2.2.29 counter never incremented while running");
        syscall.shutdown();
    }
    ...
    if (suspend_ret < 0) {
        t.failWithVal("§2.2.29 thread_suspend", 0, suspend_ret);
        syscall.shutdown();
    }
    ...
    if (after_wait != after_suspend) {
        t.fail("§2.2.29 counter changed while suspended");
        syscall.shutdown();
    }
    ...
    if (resume_ret < 0) {
        t.failWithVal("§2.2.29 thread_resume", 0, resume_ret);
        syscall.shutdown();
    }
    ...
    if (after_resume > after_wait) {
        ...
        t.fail("§2.2.29 counter did not increment after resume");
    }
