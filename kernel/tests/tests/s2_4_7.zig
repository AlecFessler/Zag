...
/// §2.4.7 — Virtual BAR write: the kernel executes the port write using the value decoded from the faulting instruction.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const pio = t.requirePioDevice(view, "§2.4.7");
    ...
    if (ret != 0) {
        t.failWithVal("§2.4.7 mem_mmio_map", 0, ret);
        syscall.shutdown();
    }
