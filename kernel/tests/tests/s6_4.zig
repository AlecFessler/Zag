...
/// §6.4 — Demand-paged private region: allocate zeroed page, map, resume.
///
...
pub fn main(_: u64) void {
    ...
    while (p < N_PAGES) : (p += 1) {
        ...
        if (ptr.* != 0) {
            t.fail("§6.4 non-zero demand page");
            syscall.shutdown();
        }
    }
    ...
    while (p < N_PAGES) : (p += 1) {
        ...
        if (ptr.* != 0xABCD_0000 + p) {
            t.failWithVal("§6.4 sentinel mismatch", @bitCast(0xABCD_0000 + p), @bitCast(ptr.*));
            syscall.shutdown();
        }
    }
