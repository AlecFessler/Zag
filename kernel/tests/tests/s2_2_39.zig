...
/// §2.2.39 — Revoking a thread handle removes it from the permissions table without affecting the thread's execution or state
pub fn main(pv: u64) void {
    ...
    if (handle_ret <= 0) {
        t.failWithVal("§2.2.39 thread_create", 1, handle_ret);
        syscall.shutdown();
    }
    ...
    if (!found_before) {
        t.fail("§2.2.39 handle not found before revoke");
        syscall.shutdown();
    }
    ...
    if (revoke_ret != 0) {
        t.failWithVal("§2.2.39 revoke_perm", 0, revoke_ret);
        syscall.shutdown();
    }
    ...
    if (found_after) {
        t.fail("§2.2.39 handle still present after revoke");
        syscall.shutdown();
    }
    ...
    if (after > before) {
        ...
        t.fail("§2.2.39 thread stopped after revoke");
    }
    syscall.shutdown();
}
