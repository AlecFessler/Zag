...
pub fn main(pv: u64) void {
    ...
    if (found_faulter) {
        t.fail("§2.12.24 A faulter handle not removed");
        syscall.shutdown();
    }
    if (view[mt_slot].entry_type != perm_view.ENTRY_TYPE_PROCESS) {
        t.fail("§2.12.24 A child died (should stay alive)");
        syscall.shutdown();
    }
    if (siblings_after == 0 or siblings_after < siblings_before) {
        t.failWithVal("§2.12.24 A siblings lost", @bitCast(@as(u64, siblings_before)), @bitCast(@as(u64, siblings_after)));
        syscall.shutdown();
    }
    ...
    if (view[single_slot].entry_type != perm_view.ENTRY_TYPE_DEAD_PROCESS) {
        t.fail("§2.12.24 B child did not become dead_process");
        syscall.shutdown();
    }
