/// §4.42.1 — `vm_reply` returns `E_OK` on success.
...
// VmReplyAction for kill (action_type=4, no payload).
...
pub fn main(pv: u64) void {
    ...
    if (cr != syscall.E_OK) {
        t.failWithVal("§4.42.1 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§4.42.1 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.42.1 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.42.1 set_state", syscall.E_OK, sr);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    if (exit_token <= 0) {
        t.failWithVal("§4.42.1 recv", 1, exit_token);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }
    ...
    const result = syscall.vm_reply_action(@bitCast(exit_token), @intFromPtr(&reply_action));
    if (result != syscall.E_OK) {
        t.failWithVal("§4.42.1", syscall.E_OK, result);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }

    // Double vm_reply with the same exit_token must return E_NOENT
    // because the pending exit was already consumed by the first reply.
    const double_result = syscall.vm_reply_action(@bitCast(exit_token), @intFromPtr(&reply_action));
    t.expectEqual("§4.42.1", syscall.E_NOENT, double_result);
