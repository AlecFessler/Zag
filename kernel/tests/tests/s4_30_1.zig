...
pub fn main(_: u64) void {
    ...
    t.expectEqual("§4.30.1 suspend rc", E_OK, ret);
    ...
    if (after == snapshot) {
        t.pass("§4.30.1 worker halted after suspend");
    } else {
        t.failWithVal("§4.30.1 worker still running", @intCast(snapshot), @intCast(after));
    }
    syscall.shutdown();
}
