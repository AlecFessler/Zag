...
/// §4.1.36 — The fault box state is fully independent from the IPC message box state.
...
pub fn main(_: u64) void {
    // ---- Sub-scenario A: fault_box pending, ipc_recv must see E_AGAIN ----
    {
        ...
        if (token < 0) {
            t.failWithVal("§4.1.36 A fault_recv", 0, token);
            syscall.shutdown();
        }
        ...
        if (second_fault != E_BUSY) {
            t.failWithVal("§4.1.36 A fault_box not pending_reply", E_BUSY, second_fault);
            syscall.shutdown();
        }
        ...
        if (ipc_rc != E_AGAIN) {
            t.failWithVal("§4.1.36 A ipc_recv leaked fault_box state", E_AGAIN, ipc_rc);
    ...
    var scenarioB_caller_handle: u64 = 0;
    {
        ...
        if (recv_rc != 0) {
            t.failWithVal("§4.1.36 B ipc_recv", 0, recv_rc);
            syscall.shutdown();
        }
        ...
        if (second_ipc != E_BUSY) {
            t.failWithVal("§4.1.36 B msg_box not pending_reply", E_BUSY, second_ipc);
            syscall.shutdown();
        }
        ...
        if (fr_rc != E_AGAIN) {
            t.failWithVal("§4.1.36 B fault_recv leaked msg_box state", E_AGAIN, fr_rc);
            _ = syscall.ipc_reply(&.{});
            syscall.shutdown();
        }
        ...
        // child and receiving it. Per §4.1.36 this is explicitly allowed.
        ...
        if (c_token < 0) {
            t.failWithVal("§4.1.36 C fault_recv", 0, c_token);
            _ = syscall.ipc_reply(&.{});
            syscall.shutdown();
        }
        ...
        if (syscall.ipc_recv(false, &ignore2) != E_BUSY) {
            t.fail("§4.1.36 C msg_box lost pending_reply");
        ...
        if (syscall.fault_recv(@intFromPtr(&fb2), 0) != E_BUSY) {
            t.fail("§4.1.36 C fault_box lost pending_reply");
            _ = syscall.fault_reply_simple(@bitCast(c_token), syscall.FAULT_KILL);
            _ = syscall.ipc_reply(&.{});
            syscall.shutdown();
        }
        ...
        if (syscall.ipc_recv(false, &ignore3) != E_BUSY) {
            t.fail("§4.1.36 C fault_reply corrupted msg_box");
            _ = syscall.ipc_reply(&.{});
            syscall.shutdown();
        }
