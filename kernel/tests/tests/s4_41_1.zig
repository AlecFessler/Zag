/// §4.41.1 — `vm_recv` returns the exit token (positive u64) on success.
...
pub fn main(pv: u64) void {
    ...
    if (cr < 0) {
        t.failWithVal("§4.41.1 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§4.41.1 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.41.1 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.41.1 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit_token > 0) {
        ...
        t.failWithVal("§4.41.1", 1, exit_token);
    }
