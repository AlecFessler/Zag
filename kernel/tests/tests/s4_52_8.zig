...
/// §4.52.8 — Counter entries beyond `PmuInfo.num_counters` in the returned `PmuSample` are zero.
pub fn main(_: u64) void {
    ...
    while (i < syscall.PMU_MAX_COUNTERS) : (i += 1) {
        if (sample.counters[i] != 0) {
            t.fail("§4.52.8 slot beyond num_counters not zero");
            _ = syscall.thread_kill(worker_h);
            syscall.shutdown();
        }
    }
