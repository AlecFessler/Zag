...
/// §4.1.23 — On any `fault_reply`, all threads in the target process that are in `.suspended` state are moved to `.ready` and re-enqueued before the action on the faulting thread is applied.
pub fn main(_: u64) void {
    ...
    if (token < 0) {
        t.failWithVal("§4.1.23 fault_recv", 0, token);
        syscall.shutdown();
    }
    ...
    if (still != before) {
        t.fail("§4.1.23 worker not suspended pre-reply");
    ...
    if (rc != 0) {
        t.failWithVal("§4.1.23 fault_reply", 0, rc);
        syscall.shutdown();
    }
    ...
    if (token2 < 0) {
        t.failWithVal("§4.1.23 fault_recv 2", 0, token2);
        syscall.shutdown();
    }
    ...
    if (after > still) {
        ...
        t.fail("§4.1.23 worker did not resume");
    }
