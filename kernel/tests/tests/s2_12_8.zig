...
pub fn main(pv: u64) void {
    ...
    // §2.12.8 holds, fault_recv returns the fault token, proving (a) a fault
    ...
    if (call_rc != 0) {
        t.failWithVal("§2.12.8 ipc_call", 0, call_rc);
        syscall.shutdown();
    }

    const token: i64 = @bitCast(reply.words[0]);
    if (token <= 0) {
        t.failWithVal("§2.12.8 fault_recv token", 1, token);
        syscall.shutdown();
    }
    ...
    if (view[slot].entry_type != perm_view.ENTRY_TYPE_PROCESS) {
        t.fail("§2.12.8 child died");
        syscall.shutdown();
    }
    if (view[slot].processRestartCount() != 0) {
        t.fail("§2.12.8 child restarted (should not have)");
        syscall.shutdown();
    }
