...
/// §4.1.27 — `fault_reply` with `FAULT_EXCLUDE_NEXT` sets `exclude_oneshot` on the faulting thread's perm entry in the handler's table and clears `exclude_permanent`.
pub fn main(pv: u64) void {
    ...
    // `exclude_oneshot` before we see it.
    ...
    if (token < 0) {
        t.fail("§4.1.27 fault_recv failed");
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, token_u)) |e| {
        if (!e.threadExcludePermanent()) {
            t.fail("§4.1.27 setup: exclude_permanent not visible");
            syscall.shutdown();
        }
    }
    ...
    if (rc != 0) {
        t.failWithVal("§4.1.27 fault_reply rc", 0, rc);
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, token_u)) |e| {
        if (e.threadExcludeOneshot() and !e.threadExcludePermanent()) {
            ...
            t.fail("§4.1.27 wrong flag state after FAULT_EXCLUDE_NEXT");
        }
    } else {
        t.fail("§4.1.27 thread entry vanished");
    }
