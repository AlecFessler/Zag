...
/// §3.3.14 — `reply` to a `call` copies reply payload to the caller's registers and unblocks the caller.
///
...
pub fn main(_: u64) void {
    ...
    if (!observed_blocked) {
        t.fail("§3.3.14 call not observed blocked before reply");
        syscall.shutdown();
    }
    ...
    if (call_result == 0 and reply_val == 0x43) {
        ...
        t.failWithVal("§3.3.14", 0, call_result);
    }
    syscall.shutdown();
}
