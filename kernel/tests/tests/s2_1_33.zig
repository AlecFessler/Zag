...
/// §2.1.33 — All threads are removed on restart; only a fresh initial thread runs.
///
...
pub fn main(pv_arg: u64) void {
    ...
    if (post_count != 1) {
        t.failWithVal("§2.1.33 post_count", 1, @intCast(post_count));
        syscall.shutdown();
    }
    ...
    if (pre_count == 0 or tid_reused) {
        t.fail("§2.1.33 pre tid reused or missing");
        syscall.shutdown();
    }
