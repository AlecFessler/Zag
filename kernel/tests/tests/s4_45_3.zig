/// §4.45.3 — `vcpu_run` when the vCPU is not in `idle` state returns `E_BUSY`.
...
pub fn main(pv: u64) void {
    ...
    if (cr != syscall.E_OK) {
        t.failWithVal("§4.45.3 create", syscall.E_OK, cr);
        syscall.shutdown();
    }

    const vcpu_handle = findVcpuHandle(view, self_handle);
    if (vcpu_handle == 0) {
        t.fail("§4.45.3 no vCPU handle");
    ...
    if (run_result != syscall.E_OK) {
        t.failWithVal("§4.45.3 vcpu_run", syscall.E_OK, run_result);
        _ = syscall.vm_destroy();
        syscall.shutdown();
    }

    // Immediately try vcpu_run again while vCPU is not idle — should return E_BUSY.
    const result = syscall.vcpu_run(vcpu_handle);
    t.expectEqual("§4.45.3", syscall.E_BUSY, result);
