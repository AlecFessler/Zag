...
fn faultedSubtest() void {
    ...
    // the second half of §2.14.11 (the `.faulted` positive path). On
    ...
    if (token < 0) {
        t.failWithVal("§2.14.11 faulted fault_recv", 0, token);
        syscall.shutdown();
    }
    ...
    if (rc != syscall.E_OK) {
        t.failWithVal("§2.14.11 faulted pmu_read", syscall.E_OK, rc);
...
///       is the `.faulted` half of the §2.14.11 positive path that was
