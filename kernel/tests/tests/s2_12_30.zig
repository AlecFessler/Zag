...
pub fn main(pv: u64) void {
    ...
    if (thread_handle == 0) {
        t.fail("§2.12.30 no thread handle");
        syscall.shutdown();
    }
    ...
    if (rc != 0) {
        t.failWithVal("§2.12.30 exclude_next rc", 0, rc);
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, thread_handle)) |e| {
        if (e.threadExcludeOneshot() and !e.threadExcludePermanent()) {
            ...
            t.fail("§2.12.30 wrong flag state after exclude_next");
        }
    } else {
        t.fail("§2.12.30 thread entry vanished");
    }
    syscall.shutdown();
}
