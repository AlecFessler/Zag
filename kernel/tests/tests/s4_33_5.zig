...
/// §4.33.5 — `fault_recv` returns `E_BUSY` if the fault box is already in `pending_reply` state
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.33.5 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.33.5 first fault_recv", 0, recv_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.33.5", E_BUSY, ret2);
