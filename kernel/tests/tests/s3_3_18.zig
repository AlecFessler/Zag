...
/// §3.3.18 — The call wait queue is priority ordered (highest priority first), with FIFO ordering among callers of the same priority level.
///
...
pub fn main(_: u64) void {
    ...
    if (r_high == 1 and r_idle == 2) {
        ...
        t.failWithVal("§3.3.18", 1, @bitCast(r_high));
    }
    syscall.shutdown();
}
