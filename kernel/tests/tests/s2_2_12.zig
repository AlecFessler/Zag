...
/// §2.2.12 — Revoking a thread handle via `revoke_perm` removes the handle from the permissions table without killing or suspending the thread
pub fn main(pv: u64) void {
    ...
    if (handle_ret <= 0) {
        t.failWithVal("§2.2.12 thread_create", 1, handle_ret);
        syscall.shutdown();
    }
    ...
    if (revoke_ret != 0) {
        t.failWithVal("§2.2.12 revoke_perm", 0, revoke_ret);
        syscall.shutdown();
    }
    ...
    if (found) {
        t.fail("§2.2.12 handle still in perm view after revoke");
        syscall.shutdown();
    }
    ...
    if (after > before) {
        ...
        t.fail("§2.2.12 thread stopped after handle revoke");
    }
    syscall.shutdown();
}
