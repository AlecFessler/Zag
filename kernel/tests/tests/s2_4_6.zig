...
/// inserted. When that worker thread exits, §2.4.6 requires its handle to
...
pub fn main(pv: u64) void {
    ...
    if (n0 == 0) {
        t.fail("§2.4.6 no child thread handle after fault_handler acquire");
        syscall.shutdown();
    }
    ...
    if (saw_worker_inserted and saw_worker_removed) {
        ...
        t.fail("§2.4.6 worker thread handle never appeared in our view");
    } else {
        t.fail("§2.4.6 worker thread handle never cleared from our view");
    }
    syscall.shutdown();
}
