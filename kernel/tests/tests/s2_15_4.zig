...
pub fn main(_: u64) void {
    ...
    if (syscall.sys_info(@intFromPtr(&info), @intFromPtr(&cores)) != syscall.E_OK) {
        t.fail("§2.15.4 sys_info with cores");
        syscall.shutdown();
    }
    ...
    while (i < info.core_count) : (i += 1) {
        ...
        const untouched =
            ...
            c.freq_hz == sentinel;
        if (untouched) {
            t.failWithVal("§2.15.4 entry not populated", @intCast(i), @intCast(info.core_count));
            syscall.shutdown();
        }
    ...
    if (populated_cores == 0) {
        t.fail("§2.15.4 no core has nonzero accounting");
        syscall.shutdown();
    }
    ...
    while (j < syscall.MAX_CPU_CORES) : (j += 1) {
        ...
        if (c.idle_ns != sentinel or c.busy_ns != sentinel or c.freq_hz != sentinel) {
            t.failWithVal("§2.15.4 out-of-range entry overwritten", @intCast(j), @intCast(info.core_count));
            syscall.shutdown();
        }
    }
