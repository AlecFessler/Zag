...
/// §2.3.50 — `mem_mmio_map` returns `E_OK` on success.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const dev_entry = t.requireMmioDevice(view, "§2.3.50");
    ...
    t.expectEqual("§2.3.50 rc", E_OK, ret);
    ...
    if (cap == 0xDEADBEEF) t.fail("§2.3.50 implausible BAR value");
    t.pass("§2.3.50 BAR accessible");
    syscall.shutdown();
}
