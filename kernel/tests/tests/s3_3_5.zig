...
/// §3.3.5 — `call` blocks the caller until the receiver calls `reply`.
///
...
pub fn main(_: u64) void {
    ...
    if (!observed_blocked) {
        t.fail("§3.3.5 call did not block after child observed message");
        syscall.shutdown();
    }
    ...
    if (call_result == 0 and reply_val == 0x43) {
        ...
        t.failWithVal("§3.3.5", 0, call_result);
    }
    syscall.shutdown();
}
