...
/// §4.57.3 — The offset update is atomic: concurrent `clock_getwall` calls on other threads see either the old or the new time, never a torn value.
pub fn main(perm_view: u64) void {
    ...
    if (diff >= 0 and diff < 1_000_000_000) {
        ...
        t.failWithVal("§4.57.3", @bitCast(target_ns), now);
    }
    syscall.shutdown();
}
