...
/// §6.2 — Fault on SHM/MMIO region kills with `invalid_read`/`invalid_write`/`invalid_execute` based on access type.
pub fn main(pv: u64) void {
    ...
    // SHM: write-only → reading → invalid_read.
    {
        ...
        if (reason != .invalid_read) {
            t.failWithVal("§6.2 shm invalid_read", @intFromEnum(perm_view.CrashReason.invalid_read), @intFromEnum(reason));
            passed = false;
        }
    }
    ...
    // SHM: read-only → writing → invalid_write.
    {
        ...
        if (reason != .invalid_write) {
            t.failWithVal("§6.2 shm invalid_write", @intFromEnum(perm_view.CrashReason.invalid_write), @intFromEnum(reason));
            passed = false;
        }
    }
    ...
    // SHM: read+write (no execute) → executing → invalid_execute.
    {
        ...
        if (reason != .invalid_execute) {
            t.failWithVal("§6.2 shm invalid_execute", @intFromEnum(perm_view.CrashReason.invalid_execute), @intFromEnum(reason));
            passed = false;
        }
    }
    ...
    _ = t.requireMmioDevice(view, "§6.2");

    {
        const dev = findMmioDevice(view);
        const reason = runMmioChild(view, children.child_mmio_invalid_read, dev);
        if (reason != .invalid_read) {
            t.failWithVal("§6.2 mmio invalid_read", @intFromEnum(perm_view.CrashReason.invalid_read), @intFromEnum(reason));
            passed = false;
        }
    }
    {
        const dev = findMmioDevice(view);
        if (dev == 0) {
            t.fail("§6.2 mmio device not returned");
            passed = false;
        } else {
            const reason = runMmioChild(view, children.child_mmio_invalid_write, dev);
            if (reason != .invalid_write) {
                t.failWithVal("§6.2 mmio invalid_write", @intFromEnum(perm_view.CrashReason.invalid_write), @intFromEnum(reason));
                passed = false;
            }
        }
    }
    {
        ...
        if (dev == 0) {
            ...
            if (reason != .invalid_execute) {
                t.failWithVal("§6.2 mmio invalid_execute", @intFromEnum(perm_view.CrashReason.invalid_execute), @intFromEnum(reason));
                passed = false;
            }
        }
    }
