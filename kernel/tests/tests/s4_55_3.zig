...
///
/// Both subtests live in the same file so the §4.55.3 spec tag still
/// binds to exactly one test.
pub fn main(_: u64) void {
    // (1) Null info_ptr.
    const rc_null = syscall.sys_info(0, 0);
    if (rc_null != syscall.E_BADADDR) {
        t.failWithVal("§4.55.3 null info_ptr", syscall.E_BADADDR, rc_null);
        syscall.shutdown();
    }
    ...
    if (rc_kern != syscall.E_BADADDR) {
        t.failWithVal("§4.55.3 kernel-partition info_ptr", syscall.E_BADADDR, rc_kern);
        syscall.shutdown();
    }
