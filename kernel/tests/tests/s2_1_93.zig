...
/// §2.1.93 — `disable_restart` clears restart for all descendants recursively.
///
...
pub fn main(pv: u64) void {
    ...
    if (inter_slot == 0xFFFF) {
        t.fail("§2.1.93 intermediate not found");
        syscall.shutdown();
    }
    ...
    while (@atomicLoad(u64, counter, .seq_cst) < 3) : (spins += 1) {
        if (spins > 1_000_000) {
            t.fail("§2.1.93 grandchild counter never advanced");
            syscall.shutdown();
        }
    ...
    if (after != snap) {
        t.failWithVal("§2.1.93 grandchild still restarting", @intCast(snap), @intCast(after));
        syscall.shutdown();
    }
