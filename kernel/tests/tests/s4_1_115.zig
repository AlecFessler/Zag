...
/// §4.1.115 — `pmu_stop` on a target thread that is not the caller and not in `.faulted` or `.suspended` state returns `E_BUSY`.
///
...
pub fn main(_: u64) void {
    ...
    if (h <= 0) {
        t.failWithVal("§4.1.115 thread_create", 1, h);
        syscall.shutdown();
    }
    ...
    if (rc != syscall.E_BUSY) {
        t.failWithVal("§4.1.115", syscall.E_BUSY, rc);
        @atomicStore(u64, &worker_stop, 1, .seq_cst);
        _ = syscall.thread_kill(worker_h);
        syscall.shutdown();
    }
