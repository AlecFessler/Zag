...
/// §4.10.2 — `proc_create` child starts with `HANDLE_SELF` at slot 0 and its initial thread handle at slot 1 with rights = `thread_rights`.
pub fn main(_: u64) void {
    ...
    if (ret != 0) {
        t.failWithVal("§4.10.2 ipc_call", 0, ret);
        syscall.shutdown();
    }
    ...
    if (entry_type != perm_view.ENTRY_TYPE_THREAD) {
        t.fail("§4.10.2 slot 1 entry_type not THREAD");
        pass_all = false;
    }
    if (handle == 0) {
        t.fail("§4.10.2 slot 1 handle is zero");
        pass_all = false;
    }
    if (rights != expected_rights) {
        t.fail("§4.10.2 slot 1 rights mismatch");
        pass_all = false;
    }
