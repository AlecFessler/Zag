...
pub fn main(pv: u64) void {
    ...
    if (!saw_token) {
        t.fail("§2.12.9 first fault not delivered before §2.12.9 fired");
        syscall.shutdown();
    }
    if (!saw_restart) {
        t.fail("§2.12.9 child did not restart after all threads faulted");
        syscall.shutdown();
    }

    const reason = view[slot].processCrashReason();
    if (reason != .unmapped_access and reason != .invalid_read) {
        t.fail("§2.12.9 wrong crash reason");
        syscall.shutdown();
    }
