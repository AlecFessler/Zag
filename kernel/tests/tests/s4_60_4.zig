...
/// §4.60.4 — `irq_ack` on a device with no associated IRQ line returns `E_INVAL`.
pub fn main(pv: u64) void {
    ...
    for (0..128) |i| {
        ...
        if (rights.irq) {
            ...
            if (rc == syscall.E_INVAL or rc == syscall.E_OK) {
                ...
                t.failWithVal("§4.60.4", syscall.E_INVAL, rc);
            }
            syscall.shutdown();
        }
    }
    ...
    t.expectEqual("§4.60.4", syscall.E_BADHANDLE, rc);
    syscall.shutdown();
}
