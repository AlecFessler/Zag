...
/// §2.14.13 — On a PMU overflow fault, a profiler typically calls `pmu_read` to retrieve the final counter values, `pmu_reset` to reconfigure counters with the next threshold, and `fault_reply` with `FAULT_RESUME` to resume the thread.
pub fn main(_: u64) void {
    ...
    if (token < 0) {
        t.failWithVal("§2.14.13 fault_recv", 0, token);
        syscall.shutdown();
    }
    ...
    if (read_rc != syscall.E_OK) {
        t.failWithVal("§2.14.13 pmu_read", syscall.E_OK, read_rc);
    ...
    if (reset_rc != syscall.E_OK) {
        t.failWithVal("§2.14.13 pmu_reset", syscall.E_OK, reset_rc);
        _ = syscall.fault_reply_simple(token_u, syscall.FAULT_KILL);
        syscall.shutdown();
    }
    ...
    if (reply_rc != syscall.E_OK) {
        t.failWithVal("§2.14.13 fault_reply", syscall.E_OK, reply_rc);
        syscall.shutdown();
    }
