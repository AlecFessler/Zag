...
/// §2.4.26 — When a pinned thread blocks (on a futex or IPC recv), it temporarily releases its core.
///
...
pub fn main(_: u64) void {
    ...
    if (pin_ret <= 0) {
        t.failWithVal("§2.4.26 pin failed", 1, pin_ret);
        syscall.shutdown();
    }
    ...
    if (th <= 0) {
        t.failWithVal("§2.4.26 thread_create", 1, th);
        syscall.shutdown();
    }
    ...
    if (ran == 1) {
        t.pass("§2.4.26 helper ran on released core");
    } else {
        t.fail("§2.4.26 helper did not run while pinned thread blocked");
    }
