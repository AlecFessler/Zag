...
/// §4.34.3 — `fault_reply` returns `E_NOENT` if `fault_token` does not match the currently pending thread
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.34.3 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (token_ret < 0) {
        t.failWithVal("§4.34.3 fault_recv", 0, token_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.34.3", E_NOENT, reply_ret);
