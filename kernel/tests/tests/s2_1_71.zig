...
/// §2.1.71 — User address space spans `[0, 0xFFFF_8000_0000_0000)`.
pub fn main(_: u64) void {
    ...
    if (ok.val <= 0 or ok.val2 == 0 or ok.val2 >= USER_END) {
        t.fail("§2.1.71 default reservation outside user space");
        syscall.shutdown();
    }
    ...
    if (at_bound.val > 0) {
        ...
        if (at_bound.val2 >= USER_END or at_bound.val2 == USER_END) {
            t.fail("§2.1.71 reserve @ USER_END produced kernel-half mapping");
            syscall.shutdown();
        }
        if (at_bound.val2 == USER_END) {
            t.fail("§2.1.71 hint @ USER_END was honoured");
            syscall.shutdown();
        }
    ...
    if (beyond.val > 0) {
        if (beyond.val2 >= USER_END) {
            t.fail("§2.1.71 reserve beyond USER_END produced kernel-half mapping");
            syscall.shutdown();
        }
