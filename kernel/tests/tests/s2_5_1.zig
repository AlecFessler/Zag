...
/// §2.5.1 — Device access is exclusive (only one process holds the handle at a time).
...
pub fn main(pv: u64) void {
    ...
    const dev = t.requireMmioDevice(view, "§2.5.1");
    ...
    if (rc == E_BADHANDLE) {
        ...
        t.failWithVal("§2.5.1 after_transfer", E_BADHANDLE, rc);
    }
    syscall.shutdown();
}
