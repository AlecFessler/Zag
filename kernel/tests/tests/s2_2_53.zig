...
/// §2.2.53 — `thread_suspend` on a thread in `.exited` state returns `E_BADHANDLE`.
///
...
pub fn main(pv: u64) void {
    ...
    if (thread_handle <= 0) {
        t.failWithVal("§2.2.53 thread_create", 1, thread_handle);
        syscall.shutdown();
    }
    ...
    if (slot == 0xFFFF) {
        t.fail("§2.2.53 thread slot not found");
        syscall.shutdown();
    }
