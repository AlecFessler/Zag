...
/// §4.1.63 — `fault_read_mem` with invalid or wrong-type `proc_handle` returns `E_BADHANDLE`
pub fn main(_: u64) void {
    var buf: [8]u8 = undefined;

    // Test with a garbage handle value.
    const ret1 = syscall.fault_read_mem(0xDEADBEEF, 0x1000, @intFromPtr(&buf), 8);
    t.expectEqual("§4.1.63 garbage handle", E_BADHANDLE, ret1);
    ...
    if (self_thread > 0) {
        ...
        t.expectEqual("§4.1.63 wrong type (thread)", E_BADHANDLE, ret2);
    } else {
        t.fail("§4.1.63 could not get thread_self");
    }
