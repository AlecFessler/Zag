...
pub fn main(_: u64) void {
    ...
    if (worker <= 0) {
        t.failWithVal("§4.1.43 thread_create", 1, worker);
        syscall.shutdown();
    }
    ...
    if (rd != syscall.E_OK) {
        t.failWithVal("§4.1.43 pmu_read", syscall.E_OK, rd);
    ...
    if (sample_ts < ts_before or sample_ts > ts_after) {
        t.failWithVal("§4.1.43 timestamp out of bracket", ts_before, sample_ts);
        _ = syscall.thread_kill(worker_h);
        syscall.shutdown();
    }
