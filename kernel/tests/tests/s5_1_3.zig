...
/// §5.1.3 — `clock_setwall` atomically updates the wall clock offset so that subsequent `clock_getwall` calls reflect the new time.
pub fn main(perm_view: u64) void {
    ...
    if (diff >= 0 and diff < 1_000_000_000) {
        ...
        t.failWithVal("§5.1.3", @bitCast(target_ns), now);
    }
    syscall.shutdown();
}
