...
/// §2.8.4 — Fault on the underflow guard (below stack) kills with fault reason `stack_overflow` (§3).
pub fn main(pv: u64) void {
    ...
    t.expectEqual("§2.8.4", @intFromEnum(perm_view.CrashReason.stack_overflow), @as(i64, @intFromEnum(view[slot].processCrashReason())));
    syscall.shutdown();
}
