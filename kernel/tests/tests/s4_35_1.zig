...
/// §4.35.1 — `fault_read_mem` returns `E_OK` on success
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.35.1 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.35.1 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }
    ...
    if (proc_handle == 0) {
        t.fail("§4.35.1 no fault_handler proc handle");
        syscall.shutdown();
    }
