...
/// §4.37.3 — `fault_set_thread_mode` with invalid or wrong-type `thread_handle` returns `E_BADHANDLE`
pub fn main(_: u64) void {
    // Test with a garbage handle value.
    const ret1 = syscall.fault_set_thread_mode(0xDEADBEEF, syscall.FAULT_MODE_STOP_ALL);
    t.expectEqual("§4.37.3 garbage handle", E_BADHANDLE, ret1);
    ...
    t.expectEqual("§4.37.3 wrong type (process)", E_BADHANDLE, ret2);
