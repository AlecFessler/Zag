...
/// §2.4.18 — The typical driver flow is: read `field0`, call `futex_wait_val` on the `field0` address with the current value as expected (the thread wakes when bit 16 is set by the kernel), handle the interrupt in userspace, call `irq_ack` to clear the pending bit and unmask the line.
pub fn main(pv: u64) void {
    ...
    for (0..128) |i| {
        ...
        if (ret == syscall.E_TIMEOUT) {
            ...
            t.failWithVal("§2.4.18", syscall.E_TIMEOUT, ret);
        }
        syscall.shutdown();
    }
