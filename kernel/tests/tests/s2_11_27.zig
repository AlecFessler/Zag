...
pub fn main(pv: u64) void {
    ...
    if (match_count != 1) {
        t.failWithVal("§2.11.27 match count", 1, @intCast(match_count));
        syscall.shutdown();
    }
    if (match_handle == ch) {
        t.fail("§2.11.27 inserted handle equals ch");
        syscall.shutdown();
    }
    if (match_rights != expected_rights) {
        t.failWithVal("§2.11.27 rights", @intCast(expected_rights), @intCast(match_rights));
        syscall.shutdown();
    }
