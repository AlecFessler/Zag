...
pub fn main(pv_addr: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§4.15.4 pin", 1, ret);
        syscall.shutdown();
    }
    t.pass("§4.15.4 pin returns handle");
    ...
    for (0..128) |i| {
        ...
        if (e.entry_type == perm_view.ENTRY_TYPE_CORE_PIN and e.handle == handle) {
            if (e.field0 == 1) {
                t.pass("§4.15.4 core_pin field0=1");
            } else {
                t.failWithVal("§4.15.4 core_pin field0", 1, @bitCast(e.field0));
            }
            found = true;
            break;
        }
    ...
    if (!found) {
        t.fail("§4.15.4 core_pin entry not found");
    }
