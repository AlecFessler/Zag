...
/// §2.5.3 — Device user view `field1` encodes: `pci_vendor(u16) | pci_device(u16) << 16 | pci_class(u8) << 32 | pci_subclass(u8) << 40`.
...
pub fn main(pv: u64) void {
    ...
    const dev = t.requireMmioDevice(view, "§2.5.3");
    ...
    if (dev.pciVendor() != expected_vendor) {
        t.failWithVal("§2.5.3 vendor", expected_vendor, dev.pciVendor());
        syscall.shutdown();
    }
    if (dev.pciDevice() != expected_device) {
        t.failWithVal("§2.5.3 device", expected_device, dev.pciDevice());
        syscall.shutdown();
    }
    if (dev.pciClassCode() != expected_class) {
        t.failWithVal("§2.5.3 class", expected_class, dev.pciClassCode());
        syscall.shutdown();
    }
    if (dev.pciSubclass() != expected_subclass) {
        t.failWithVal("§2.5.3 subclass", expected_subclass, dev.pciSubclass());
        syscall.shutdown();
    }
    ...
    if ((dev.field1 & 0xFFFF_FFFF_FFFF) != expected_low48) {
        t.failWithVal("§2.5.3 raw_f1", @bitCast(expected_low48), @bitCast(dev.field1 & 0xFFFF_FFFF_FFFF));
        syscall.shutdown();
    }
