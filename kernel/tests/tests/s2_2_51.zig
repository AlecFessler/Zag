...
/// §2.2.51 — `thread_suspend` on a thread in `.faulted` state returns `E_BUSY`.
pub fn main(pv: u64) void {
    ...
    if (token < 0) {
        t.fail("§2.2.51 fault_recv failed");
        syscall.shutdown();
    }
