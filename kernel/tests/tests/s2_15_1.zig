...
/// §2.15.1 — `SysInfo.core_count` is the number of active CPU cores the kernel scheduled on at boot.
///
...
///   3) stable across calls (§2.15.1 explicitly calls it a static property of
...
pub fn main(_: u64) void {
    ...
    if (rc_a != syscall.E_OK) {
        t.failWithVal("§2.15.1 first call", syscall.E_OK, rc_a);
        syscall.shutdown();
    }
    ...
    if (rc_b != syscall.E_OK) {
        t.failWithVal("§2.15.1 second call", syscall.E_OK, rc_b);
        syscall.shutdown();
    }

    if (info_a.core_count == 0) {
        t.fail("§2.15.1 core_count == 0");
        syscall.shutdown();
    }
    if (info_a.core_count > syscall.MAX_CPU_CORES) {
        t.failWithVal(
            "§2.15.1 core_count > MAX_CPU_CORES",
