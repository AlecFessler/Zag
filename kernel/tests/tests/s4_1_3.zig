...
/// §4.1.3 — Transferring `HANDLE_SELF` via capability transfer with the `fault_handler` bit set atomically: if the recipient already holds a process handle to the sender, the `fault_handler` bit is added to that existing entry; otherwise a new process handle entry is inserted into the recipient's permissions table with `fault_handler` set.
...
pub fn main(pv: u64) void {
    ...
    if (syscall.ipc_call(child_handle, &.{}, &reply) != 0) {
        t.fail("§4.1.3 first ipc_call");
        syscall.shutdown();
    }
    const pre_rights = reply.words[0];
    const fh_bit_proc: u64 = 0x80;
    if ((pre_rights & fh_bit_proc) == 0) {
        t.fail("§4.1.3 pre-transfer child missing fault_handler bit");
        syscall.shutdown();
    }
    ...
    if (syscall.ipc_call(child_handle, &.{}, &reply) != 0) {
        t.fail("§4.1.3 second ipc_call");
        syscall.shutdown();
    }
    const post_rights = reply.words[0];
    if ((post_rights & fh_bit_proc) != 0) {
        t.failWithVal("§4.1.3 sender fault_handler bit not cleared", 0, @bitCast(post_rights));
        syscall.shutdown();
    }
    ...
    if (proc_handle == 0) {
        t.fail("§4.1.3 recipient missing fault_handler entry");
        syscall.shutdown();
    }
    ...
    if (token <= 0) {
        t.failWithVal("§4.1.3 fault_recv token", 1, token);
        syscall.shutdown();
    }
    if (fault_msg.process_handle != proc_handle) {
        t.failWithVal("§4.1.3 wrong source process_handle", @bitCast(proc_handle), @bitCast(fault_msg.process_handle));
