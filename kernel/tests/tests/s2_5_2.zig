...
pub fn main(pv: u64) void {
    ...
    const mmio = t.requireMmioDevice(view, "§2.5.2 mmio");
    ...
    if (f0 != expected_f0) {
        t.failWithVal("§2.5.2 mmio_f0", @bitCast(expected_f0), @bitCast(f0));
        syscall.shutdown();
    }
    if (mmio.deviceType() != expected_type) {
        t.fail("§2.5.2 mmio_type");
        syscall.shutdown();
    }
    if (mmio.deviceClass() != expected_class) {
        t.fail("§2.5.2 mmio_class");
        syscall.shutdown();
    }
    if (mmio.deviceSizeOrPortCount() != expected_size) {
        t.fail("§2.5.2 mmio_size");
        syscall.shutdown();
    }

    const pio = t.requirePioDevice(view, "§2.5.2 pio");
    ...
    if (pf0 != pexpected_f0) {
        t.failWithVal("§2.5.2 pio_f0", @bitCast(pexpected_f0), @bitCast(pf0));
        syscall.shutdown();
    }
