...
/// garbage in the struct is caught here. §4.50.4 is "vacuously passed on
...
pub fn main(_: u64) void {
    ...
    if (info.num_counters == 0) {
        // The §4.50.4 clause: verify the required zero invariants.
        if (info.supported_events != 0 or info.overflow_support) {
            t.fail("§4.50.4 no-counter fields not zero");
            syscall.shutdown();
        }
    } else {
        // Counter-capable HW: §4.50.4 is vacuously satisfied here, so
        ...
        if (info.num_counters > syscall.PMU_MAX_COUNTERS) {
            t.failWithVal(
                "§4.50.4 num_counters > PMU_MAX_COUNTERS",
        ...
        if ((info.supported_events & ~defined_mask) != 0) {
            t.failWithVal(
                "§4.50.4 supported_events has undefined bits",
