...
/// §2.5.17 — `DeviceRegionRights.irq` gates both `irq_ack` and meaningful use of the IRQ pending bit.
pub fn main(pv: u64) void {
    ...
    for (0..128) |i| {
        ...
        if (!rights.irq) {
            ...
            t.expectEqual("§2.5.17", syscall.E_PERM, rc);
            syscall.shutdown();
        }
    }
    ...
    t.expectEqual("§2.5.17", syscall.E_BADHANDLE, rc);
    syscall.shutdown();
}
