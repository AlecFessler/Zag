...
/// §4.1.51 — `fault_recv` returns the fault token (positive u64, equal to the faulting thread's handle ID in the caller's perm table) on success and writes a `FaultMessage` to `buf_ptr`.
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.51 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.1.51 fault_recv rc", 0, recv_ret);
        syscall.shutdown();
    }
    ...
    if (token_u == fault_msg.thread_handle and token_u != 0) {
        t.pass("§4.1.51 token == FaultMessage.thread_handle");
    } else {
        t.fail("§4.1.51 token / thread_handle mismatch");
        syscall.shutdown();
    }

    // Sanity-check other FaultMessage fields.
    if (fault_msg.process_handle == 0) {
        t.fail("§4.1.51 process_handle is zero");
        syscall.shutdown();
    }
    // child_fault_after_transfer does a null read → invalid_read.
    if (fault_msg.fault_reason == 0) {
        t.fail("§4.1.51 fault_reason = none");
        syscall.shutdown();
    }
    t.pass("§4.1.51 FaultMessage populated");
    syscall.shutdown();
}
