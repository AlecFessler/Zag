...
/// §2.2.27 — `set_priority` is self-only (no thread handle parameter).
///
/// Tests:
...
pub fn main(_: u64) void {
    // 1. Missing ProcessRights.set_affinity → E_PERM.
    {
        ...
        if (rc != 0) {
            t.failWithVal("§2.2.27 case1 ipc_call", 0, rc);
            syscall.shutdown();
        }
        const child_result: i64 = @bitCast(reply.words[0]);
        if (child_result != E_PERM) {
            t.failWithVal("§2.2.27 missing ProcessRights E_PERM", E_PERM, child_result);
            syscall.shutdown();
        }
    }
    ...
    // 2. max_thread_priority=normal, request high → E_PERM (exceeds ceiling).
    {
        ...
        const rc = syscall.ipc_call(ch, &.{syscall.PRIORITY_HIGH}, &reply);
        if (rc != 0) {
            t.failWithVal("§2.2.27 case2 ipc_call", 0, rc);
            syscall.shutdown();
        }
        const child_result: i64 = @bitCast(reply.words[0]);
        if (child_result != E_PERM) {
            t.failWithVal("§2.2.27 ceiling exceeded E_PERM", E_PERM, child_result);
            syscall.shutdown();
        }
    }
    ...
    // 3. max_thread_priority=high, request high → success.
    {
        ...
        const rc = syscall.ipc_call(ch, &.{syscall.PRIORITY_HIGH}, &reply);
        if (rc != 0) {
            t.failWithVal("§2.2.27 case3 ipc_call", 0, rc);
            syscall.shutdown();
        }
        const child_result: i64 = @bitCast(reply.words[0]);
        if (child_result < 0) {
            t.failWithVal("§2.2.27 within ceiling should succeed", 0, child_result);
            syscall.shutdown();
        }
    }
