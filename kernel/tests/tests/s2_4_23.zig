...
/// §2.4.23 — When a thread calls `set_priority(.pinned)`, the kernel scans the thread's current affinity mask in ascending core ID order for a core with no pinned owner.
///
...
pub fn main(pv: u64) void {
    ...
    if (ret <= 0) {
        t.failWithVal("§2.4.23 pin failed", 1, ret);
        syscall.shutdown();
    }
    ...
    for (0..128) |i| {
        ...
        if (e.entry_type == perm_view.ENTRY_TYPE_CORE_PIN and e.handle == pin_handle) {
            if (e.field0 == 1) {
                t.pass("§2.4.23 pinned to lowest core in mask");
            } else {
                t.failWithVal("§2.4.23 expected core 1", 1, @bitCast(e.field0));
            }
            found = true;
            break;
        }
    ...
    if (!found) {
        t.fail("§2.4.23 core_pin entry not found in perm_view");
    }
