...
pub fn main(pv: u64) void {
    ...
    if (delta_a != 4) {
        t.failWithVal("§4.1.6 A delta pre-revoke", 4, @bitCast(@as(u64, delta_a)));
        syscall.shutdown();
    }
    ...
    if (fh_handle_a == 0) {
        t.fail("§4.1.6 A fh handle missing");
        syscall.shutdown();
    }
    ...
    if (delta_after_a != 0) {
        t.failWithVal("§4.1.6 A delta post-revoke", 0, @bitCast(@as(u64, delta_after_a)));
        syscall.shutdown();
    }
    ...
    if (delta_b == 0) {
        t.fail("§4.1.6 B no delta on second acquisition");
        syscall.shutdown();
    }
    ...
    if (delta_after_b != 0) {
        t.failWithVal("§4.1.6 B delta post-revoke", 0, @bitCast(@as(u64, delta_after_b)));
        syscall.shutdown();
    }
