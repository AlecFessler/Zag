...
pub fn handleResponse(pkt: []const u8, len: u32) void {
    ...
    if (msg_type == DHCP_OFFER and main.dhcp_client_state == .discovering) {
        ...
        sendRequest();
    } else if (msg_type == DHCP_ACK and (main.dhcp_client_state == .requesting or main.dhcp_client_state == .bound)) {
...
pub fn tick() void {
    ...
    if (main.dhcp_client_state == .bound) {
        ...
        if (now -| main.dhcp_client_bound_ns > t1) {
            ...
            sendRequest();
            // Stay in .requesting so we accept the ACK but keep working
            main.dhcp_client_state = .requesting;
    ...
    // Timeout for discover/request — retry after 10s
