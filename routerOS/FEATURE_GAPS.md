...
What's already implemented and working: IPv4/IPv6 dual-stack forwarding, NAT with TCP state tracking, firewall with port forwarding, DHCP server & client, DHCPv6 prefix delegation, DNS relay & caching, ARP/NDP, SLAAC, ICMP/ICMPv6 (ping, traceroute), IP fragmentation tracking, TCP MSS clamping, HTTP management API, serial console CLI, NTP, NFS client, UDP forwarding.
...
### 1. Static DHCP Leases (MAC-to-IP Reservation)

**What it is:** The ability to permanently assign a specific IP address to a specific device based on its MAC address, so it always gets the same IP even after rebooting.

**Why you'd want it:** Without this, devices like NAS boxes, printers, security cameras, or anything you've set up port forwarding for can silently change IP addresses after a lease expires. Your port forwarding rules break, your NFS mounts break, your bookmarks to local services break. Every consumer router has this. Your DHCP server currently auto-assigns from 10.1.1.100+ with no way to pin a MAC to a specific address.

### 2. Persistent Configuration Across Reboots

**What it is:** Saving router state (firewall rules, port forwards, DHCP reservations, DNS settings, timezone) to non-volatile storage so they survive a power cycle.

**Why you'd want it:** Right now, all configuration lives in RAM. If the router loses power or reboots, every firewall rule, port forward, and setting you've configured through the console or HTTP API is gone. You'd have to reconfigure everything from scratch each time. For a device that should run unattended for months, this is essential. You already have an NFS client — writing config to NFS on change and reading it back on boot is one approach.

### 3. UPnP IGD / NAT-PMP / PCP

**What it is:** Protocols that let devices on your LAN automatically request port forwards from the router without manual configuration. UPnP IGD (Internet Gateway Device) is the most common, NAT-PMP is Apple's version, and PCP (Port Control Protocol) is the modern IETF standard that supersedes both.

**Why you'd want it:** Game consoles (Xbox, PlayStation, Switch), voice/video calling apps (Discord, Zoom, FaceTime), torrent clients, and many smart home devices all use UPnP to automatically open ports. Without it, you'll get "strict NAT" warnings in games (meaning matchmaking won't work properly), and many peer-to-peer features will fail or fall back to slower relay servers. You'd need to manually set up port forwards for every application that needs inbound connections, which is impractical.
...
### 7. mDNS/DNS-SD Relay (Multicast DNS)
...
### 11. DNS over TLS (DoT) or DNS over HTTPS (DoH)
...
### 15. Single-Connection TCP Stack
...
### 18. No Management Authentication
