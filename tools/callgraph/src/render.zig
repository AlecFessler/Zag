...
/// Compact JSON renderer for the trace tree. Mirrors the same pruning
/// rules as `renderTrace` (depth cap, recursion guard, debug/library
/// folding) so output stays in sync with the textual form.
///
/// Top level: `{"stats":{...},"root":<node>}`
///
/// Node shape (terse — single-char keys, defaults elided):
///   {"n":"name","f":"file","l":line,"i":1?,"s":[siteFile,siteLine]?,"ek":"…"?,"a":[<atom>...]}
///   - "s" is the site of the call FROM the caller (omitted on root)
///   - "ek" is the edge kind, omitted when "direct"
///   - "i":1 marks the function as inlined (is_ast_only)
///   - "a" holds child atoms (calls + branches + loops)
///
/// Leaf shape (folded debug/stdlib, depth cap, recursion stop, indirect):
///   {"n":"name","s":[siteFile,siteLine],"k":"cap|rec|deb|std|ind|unr","f":"file"?,"l":line?}
///   - "f","l" present only when known (cap/rec point at the target def)
///
/// Branch:
///   {"b":"if_else|switch","s":[siteFile,siteLine],"arms":[{"L":"label","a":[<atom>...]}]}
/// Loop:
///   {"L":1,"s":[siteFile,siteLine],"a":[<atom>...]}
///
/// Discriminator rules (cheap to parse, no extra "k" field on the common
/// descended-call case): a node is a *call* if "n" is present and "k" is
/// absent; a *leaf* if "k" is present; a *branch* if "b" is present; a
/// *loop* if "L" is present.
pub fn renderTraceJson(
    arena: std.mem.Allocator,
    out: *std.io.Writer,
    ctx: Ctx,
    root: *const Function,
    ...
    try out.writeAll("{\"stats\":{");
    try out.print("\"fns\":{d},\"at_cap\":{d},\"depth\":{d}", .{
        stats.fns_visited, stats.at_cap, max_depth,
    });
    if (stats.top_fanout > 0) {
        try out.writeAll(",\"top\":{\"n\":");
        try writeJsonString(out, stats.top_name);
        try out.writeAll(",\"f\":");
        try writeJsonString(out, shortFile(stats.top_loc.file));
        try out.print(",\"l\":{d},\"c\":{d}}}", .{ stats.top_loc.line, stats.top_fanout });
    }
    try out.writeAll("},\"root\":");

    var visited = std.AutoHashMap(FnId, void).init(arena);
    try jsonFn(out, ctx, root, null, .direct, 0, max_depth, &visited);
    try out.writeAll("}\n");
}

/// Emit a function node. `site_opt` is the caller's call-site (null for
/// the root, since there's no caller). `edge_kind` is the kind of the
/// edge that brought us here ("direct" for root).
fn jsonFn(
    out: *std.io.Writer,
    ctx: Ctx,
    f: *const Function,
    site_opt: ?SourceLoc,
    edge_kind: types.EdgeKind,
    depth: u32,
    max_depth: u32,
    visited: *std.AutoHashMap(FnId, void),
) RenderError!void {
    try out.writeAll("{\"n\":");
    try writeJsonString(out, f.name);
    try out.writeAll(",\"f\":");
    try writeJsonString(out, shortFile(f.def_loc.file));
    try out.print(",\"l\":{d}", .{f.def_loc.line});
    if (f.is_ast_only) try out.writeAll(",\"i\":1");
    if (site_opt) |s| try writeJsonSite(out, s);
    if (edge_kind != .direct) try out.print(",\"ek\":\"{s}\"", .{@tagName(edge_kind)});
    try out.writeAll(",\"a\":[");
    var first = true;
    for (f.intra) |atom| {
        if (!first) try out.writeAll(",");
        first = false;
        try jsonAtom(out, ctx, atom, depth + 1, max_depth, visited);
    }
    try out.writeAll("]}");
}
...
fn jsonAtom(
    ...
    switch (atom) {
        .call => |c| try jsonCall(out, ctx, c, depth, max_depth, visited),
        .branch => |b| try jsonBranch(out, ctx, b, depth, max_depth, visited),
        .loop => |l| try jsonLoop(out, ctx, l, depth, max_depth, visited),
    }
}
...
fn jsonCall(
    ...
    if (ctx.hide_debug and isDebugName(c.name)) {
        return jsonLeaf(out, c.name, c.site, "deb", null);
    }
    ...
    if (fp == null) {
        const k: []const u8 = if (c.kind == .indirect) "ind" else "unr";
        return jsonLeaf(out, c.name, c.site, k, null);
    }
    const f = fp.?;

    if (ctx.excludes.len > 0 and matchExclude(f.name, ctx.excludes)) {
        return jsonLeaf(out, f.name, c.site, "exc", null);
    }
    if (ctx.hide_library and isLibrary(f.name)) {
        return jsonLeaf(out, f.name, c.site, "std", null);
    }
    if (visited.contains(f.id)) {
        return jsonLeaf(out, f.name, c.site, "rec", null);
    }
    if (depth + 1 >= max_depth) {
        return jsonLeaf(out, f.name, c.site, "cap", f.def_loc);
    }

    try visited.put(f.id, {});
    defer _ = visited.remove(f.id);
    try jsonFn(out, ctx, f, c.site, c.kind, depth, max_depth, visited);
}
...
fn jsonLeaf(
    ...
    site: SourceLoc,
    k: []const u8,
    target_def: ?SourceLoc,
) RenderError!void {
    try out.writeAll("{\"n\":");
    try writeJsonString(out, name);
    try writeJsonSite(out, site);
    try out.print(",\"k\":\"{s}\"", .{k});
    if (target_def) |def| {
        try out.writeAll(",\"f\":");
        try writeJsonString(out, shortFile(def.file));
        try out.print(",\"l\":{d}", .{def.line});
    }
    try out.writeAll("}");
}
...
fn jsonBranch(
    ...
    try out.print("{{\"b\":\"{s}\"", .{tag});
    try writeJsonSite(out, b.loc);
    try out.writeAll(",\"arms\":[");
    var first_arm = true;
    for (b.arms) |arm| {
        if (!first_arm) try out.writeAll(",");
        first_arm = false;
        try out.writeAll("{\"L\":");
        try writeJsonString(out, arm.label);
        try out.writeAll(",\"a\":[");
        var first = true;
        for (arm.seq) |a| {
            if (!first) try out.writeAll(",");
            first = false;
            try jsonAtom(out, ctx, a, depth, max_depth, visited);
        }
        try out.writeAll("]}");
    }
    try out.writeAll("]}");
...
fn jsonLoop(
    ...
    try out.writeAll("{\"L\":1");
    try writeJsonSite(out, l.loc);
    try out.writeAll(",\"a\":[");
    var first = true;
    for (l.body) |a| {
        if (!first) try out.writeAll(",");
...
fn writeJsonSite(out: *std.io.Writer, loc: SourceLoc) !void {
    try out.writeAll(",\"s\":[");
    try writeJsonString(out, shortFile(loc.file));
    try out.print(",{d}]", .{loc.line});
}
