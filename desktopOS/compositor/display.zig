...
pub const Display = struct {
    ...
    pub fn init(perm_view_addr: u64) ?Display {
        ...
        if (width == 0 or height == 0) {
            syscall.write("display: invalid dimensions\n");
            return null;
        }
        ...
        if (mmio_vm.val < 0) {
            syscall.write("display: vm_reserve mmio failed\n");
            return null;
        }
        if (syscall.mmio_map(display_handle, @intCast(mmio_vm.val), 0) != 0) {
            syscall.write("display: mmio_map failed\n");
            return null;
        }
        const screen_fb: [*]u32 = @ptrFromInt(mmio_vm.val2);
        ...
        if (bb_vm.val < 0) {
            syscall.write("display: vm_reserve back buffer failed\n");
            return null;
        }
        const back_buf: [*]u32 = @ptrFromInt(bb_vm.val2);

        syscall.write("compositor: display mapped\n");
