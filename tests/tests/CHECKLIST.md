...
- [ ] **07** — on success, reply handles whose suspended sender was the terminated EC return E_ABANDONED on subsequent operations.
...
- [ ] **01** — returns E_BADCAP if [1] is nonzero and not a valid EC handle.
- [ ] **02** — returns E_INVAL if any reserved bits are set in [1].
- [ ] **03** — on success, when [1] is a valid handle to a runnable EC, an observable side effect performed by the target EC (e.g., a write to shared memory) is visible to the caller before the caller's next syscall returns.
- [ ] **04** — when [1] is a valid handle, [1]'s field0 and field1 are refreshed from the kernel's authoritative state as a side effect, regardless of whether the call returns success or another error code.

## priority — 0/8

- [ ] **01** — returns E_BADCAP if [1] is not a valid EC handle.
- [ ] **02** — returns E_PERM if [1] does not have the `spri` cap.
- [ ] **03** — returns E_PERM if [2] exceeds the caller's self-handle `pri`.
- [ ] **04** — returns E_INVAL if [2] is greater than 3.
- [ ] **05** — returns E_INVAL if any reserved bits are set in [1].
- [ ] **06** — on success, when two ECs are blocked in `futex_wait_val` on the same address and a `futex_wake` is issued, the EC whose priority was last set higher via `priority` is woken first; the same ordering applies to `recv` selection when the two ECs are both queued senders on the same port.
- [ ] **07** — on success, when the target is suspended on a port or waiting on a futex, [2] takes effect on the target's next port event delivery and futex wake.
- [ ] **08** — when [1] is a valid handle, [1]'s field0 and field1 are refreshed from the kernel's authoritative state as a side effect, regardless of whether the call returns success or another error code.

## affinity — 0/6

- [ ] **01** — returns E_BADCAP if [1] is not a valid EC handle.
- [ ] **02** — returns E_PERM if [1] does not have the `saff` cap.
- [ ] **03** — returns E_INVAL if any bit set in [2] corresponds to a core the system does not have.
- [ ] **04** — returns E_INVAL if any reserved bits are set in [1].
- [ ] **05** — on success, the target EC's affinity is set to [2].
...
- [ ] **02** — [1] bits 0-7 contain the number of available PMU counters.
- [ ] **03** — [1] bit 8 is set when the hardware supports counter overflow events.
- [ ] **04** — [2] is a bitmask of supported events indexed by the table above.
...
- [ ] **09** — returns E_INVAL if props.sz is 3 (reserved).
- [ ] **10** — returns E_INVAL if props.sz exceeds caps.max_sz.
- [ ] **11** — returns E_INVAL if caps.mmio = 1 and caps.x is set.
- [ ] **12** — returns E_INVAL if caps.dma = 1 and caps.x is set.
- [ ] **13** — returns E_INVAL if caps.mmio = 1 and caps.dma = 1.
- [ ] **14** — returns E_BADCAP if caps.dma = 1 and [5] is not a valid device_region handle.
- [ ] **15** — returns E_PERM if caps.dma = 1 and [5] does not have the `dma` cap.
- [ ] **16** — returns E_INVAL if props.cur_rwx is not a subset of caps.r/w/x.
- [ ] **17** — returns E_INVAL if any reserved bits are set in [1] or [2].
- [ ] **18** — on success, the caller receives a VAR handle with caps = `[1].caps`.
- [ ] **19** — on success, field0 contains the assigned base address.
...
- [ ] **07** — on success, [1].field1 `device` is set to [2]'s handle id.
- [ ] **08** — on success, CPU accesses to the VAR's range use effective permissions = `VAR.cur_rwx`.
- [ ] **09** — when [1] is a valid handle, [1]'s field0 and field1 are refreshed from the kernel's authoritative state as a side effect, regardless of whether the call returns success or another error code.
...
- [ ] **01** — returns E_BADCAP if [1] is not a valid VAR handle.
- [ ] **02** — returns E_PERM if [1] does not have the `r` cap.
- [ ] **03** — returns E_INVAL if [2] offset is not 8-byte aligned.
- [ ] **04** — returns E_INVAL if count is 0 or count > 125.
- [ ] **05** — returns E_INVAL if [2] + count*8 exceeds the VAR's size.
- [ ] **06** — returns E_INVAL if any reserved bits are set in [1] or [2].
- [ ] **07** — on success, vregs `[3..2+count]` contain the qwords from the VAR starting at [2] offset.
- [ ] **08** — when [1] is a valid handle, [1]'s field0 and field1 are refreshed from the kernel's authoritative state as a side effect, regardless of whether the call returns success or another error code.

## idc_write — 0/8

- [ ] **01** — returns E_BADCAP if [1] is not a valid VAR handle.
- [ ] **02** — returns E_PERM if [1] does not have the `w` cap.
- [ ] **03** — returns E_INVAL if [2] offset is not 8-byte aligned.
- [ ] **04** — returns E_INVAL if count is 0 or count > 125.
- [ ] **05** — returns E_INVAL if [2] + count*8 exceeds the VAR's size.
- [ ] **06** — returns E_INVAL if any reserved bits are set in [1] or [2].
- [ ] **07** — on success, the qwords from vregs `[3..2+count]` are written into the VAR starting at [2] offset.
- [ ] **08** — when [1] is a valid handle, [1]'s field0 and field1 are refreshed from the kernel's authoritative state as a side effect, regardless of whether the call returns success or another error code.

## create_page_frame — 0/10

- [ ] **01** — returns E_PERM if the caller's self-handle lacks `crpf`.
