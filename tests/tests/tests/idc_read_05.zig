...
//   readable" shape and matches the debugger-style use of idc_read.
//
// Action
...
//      a 4 KiB VAR with caps.r = 1.
//   3. mapPf(var, &.{ 0, pf }) — must succeed; installs the backing at
//      offset 0.
//   4. idcRead(var, 0xFF8, 2) — must return E_INVAL because
//      0xFF8 + 2 * 8 = 0x1008 > 0x1000 (the VAR's byte size).
//
// Assertions
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 2: regular VAR, exactly one 4 KiB page. caps.r is required
    // for the §[idc_read] perm gate (test 02); caps.w lets the same
    // VAR receive a writable page_frame mapping in step 3 without
    ...
    // transitions 0 -> 1; the size-bound idc_read gate is independent
    // of map, but a backed VAR is the ordinary debugger-read shape.
    ...
    // Step 4: idc_read at offset 0xFF8, count = 2. 0xFF8 is 8-byte
    // aligned (test 03 clear) and count = 2 is in [1, 125] (test 04
    // clear). The read range is [0xFF8, 0x1008) — 0x1008 exceeds the
    // VAR's 0x1000-byte size by 8, which §[idc_read] test 05 demands
    // surfaces as E_INVAL.
    const result = syscall.idcRead(var_handle, 0xFF8, 2);
