...
/// §4.1.35 — When the handler process dies, all processes that had it as fault handler revert to self-fault-handling: their `fault_handler` ProcessRights bit is restored and their `fault_handler_proc` is cleared.
///
/// Spec clauses:
...
pub fn main(pv: u64) void {
    ...
    if (rc1 != 0) {
        t.failWithVal("§4.1.35 setup ipc_call", 0, rc1);
        syscall.shutdown();
    }
    ...
    if (rev_rc != 0) {
        t.failWithVal("§4.1.35 revoke handler", 0, rev_rc);
        syscall.shutdown();
    }
    ...
    if (rc3 != 0) {
        t.failWithVal("§4.1.35 report2 ipc_call", 0, rc3);
        syscall.shutdown();
    }
    const rights_after = reply3.words[0];
    if ((rights_after & fh_bit) == 0) {
        t.fail("§4.1.35 A fault_handler bit not restored after handler death");
        syscall.shutdown();
    }
    ...
    if (shm_handle_raw < 0) {
        t.fail("§4.1.35 B mem_shm_create");
        syscall.shutdown();
    }
    ...
    // Kill the middleman. Per §4.1.35 the target reverts to
    ...
    if (after_kill == before_kill) {
        t.failWithVal("§4.1.35 B worker did not resume", @bitCast(before_kill), @bitCast(after_kill));
        syscall.shutdown();
    }
