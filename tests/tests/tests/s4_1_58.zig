...
/// §4.1.58 — `fault_reply` returns `E_INVAL` if the fault box is not in `pending_reply` state, if `action` is not a valid value (0, 1, or 2), or if both `FAULT_EXCLUDE_NEXT` and `FAULT_EXCLUDE_PERMANENT` flags are set simultaneously.
pub fn main(_: u64) void {
    // --- Branch (a): no pending fault. ---
    const ret_a = syscall.fault_reply_simple(0, syscall.FAULT_KILL);
    t.expectEqual("§4.1.58 no pending", E_INVAL, ret_a);
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.58 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.1.58 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.58 invalid action", E_INVAL, ret_b);
    ...
    t.expectEqual("§4.1.58 both exclude flags", E_INVAL, ret_c);
