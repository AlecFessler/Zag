// Spec §[vm_inject_irq] vm_inject_irq — test 04.
//
// "[test 04] on success with [3].assert = 1, IRQ line [2] is
//  asserted on the VM's emulated interrupt controller; if a vCPU
//  is unmasked for the line, an interrupt event is delivered to
//  the vCPU on its next runnable opportunity (observable as an
//  exception/interrupt vm_exit or as a guest interrupt handler
//  invocation per the guest's IDT/GIC configuration)."
//
// Spec semantics
//   The full faithful witness for this property requires a guest
//   image whose IDT/GIC is configured to vector IRQ [2] into a
//   handler observable from outside the guest (via a vm_exit
//   sub-code change, MMIO ack, or HLT-resume). Booting a real
//   guest from the in-kernel test runner is not feasible: the
//   runner has no facility to stage guest code into VM memory,
//   and §[vm_policy] alone — which is all userspace can configure
//   here — does not seed an IDT/IOAPIC. We therefore exercise the
//   smoke-degraded slice the runner can witness: that on a real
//   VM with an in-range IRQ line and clean reserved bits, the
//   syscall traverses its success path and returns OK rather than
//   surfacing E_BADCAP / E_INVAL. The downstream guest-delivery
//   half of the spec assertion is unreachable from this harness
...
//   Then call vm_inject_irq(vm, irq_num=0, assert_word=1).
//
//   irq_num = 0 is in range for both x86-64's IOAPIC (24 lines)
//   and aarch64's GIC (1024 SPI/PPI/SGI ids), so §[vm_inject_irq]
//   [test 02]'s out-of-range E_INVAL gate cannot fire.
//   assert_word = 1 has bit 0 set (assert) and bits 1-63 clear,
//   so [test 03]'s reserved-bits E_INVAL gate cannot fire either.
//   With a valid VM handle in [1], [test 01]'s E_BADCAP gate is
//   also defused. The only remaining outcome on a hardware-virt
//   platform is the success path — vreg 1 == OK.
//
// Caps required
...
//      exit_port).
//   7. vmInjectIrq(vm_handle, irq_num=0, assert_word=1) — must
//      return OK in vreg 1.
//
// Assertions
...
//   4: vm_inject_irq returned non-OK on a valid VM handle with
//      irq_num = 0 and assert_word = 1.
...
pub fn main(cap_table_base: u64) void {
    ...
    //    prelude shape; both bits live within port_ceiling = 0x1C.
    ...
    // 7. Inject IRQ 0 with assert = 1. With a valid VM handle in
    //    [1], an in-range irq_num in [2], and clean reserved bits
    //    in both [1] and [3], the only spec-permitted outcome is
    //    the success path: vreg 1 == OK.
    const inject = syscall.vmInjectIrq(vm_handle, 0, 1);
    if (inject.v1 != @intFromEnum(errors.Error.OK)) {
        testing.fail(4);
        return;
    }
