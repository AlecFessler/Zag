/// §4.2.65 — `vm_intc_deassert_irq` with `irq_num` greater than or equal to 24 returns `E_INVAL`.
...
pub fn main(_: u64) void {
    ...
    const r24 = syscall.vm_intc_deassert_irq(@bitCast(cr), 24);
    if (r24 != syscall.E_INVAL) {
        t.failWithVal("§4.2.65 irq=24", syscall.E_INVAL, r24);
        passed = false;
    }

    const r99 = syscall.vm_intc_deassert_irq(@bitCast(cr), 99);
