// Spec §[terminate] terminate — test 01.
//
...
//
// Action
//   terminate(invalid_handle). terminate takes only the handle id with
//   the upper bits reserved; passing a clean (zero-extended) u12 means
//   neither the E_PERM (test 02) nor E_INVAL (test 03) check can fire.
//   The only error path that applies is BADCAP.
//
// Assertion
...
pub fn main(cap_table_base: u64) void {
    ...
    const result = syscall.terminate(empty_slot);
