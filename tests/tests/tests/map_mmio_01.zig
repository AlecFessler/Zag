...
//   arguments and still observe E_BADCAP from the [1] check. Test 02
//   covers the [2]-side BADCAP gate independently.
//
// Action
...
pub fn main(cap_table_base: u64) void {
    ...
    // table layout. The BADCAP gate on [1] must fire before [2] is
    // examined, so passing the same empty slot for [2] is harmless.
    const empty_slot: u12 = caps.HANDLE_TABLE_MAX - 1;

    const result = syscall.mapMmio(empty_slot, empty_slot);
