...
/// §2.1.86 — `proc_create` grants parent every `ProcessHandleRights` bit on the child handle except `fault_handler` (exclusive: only one process holds it for a given target, and it must be explicitly transferred via `HANDLE_SELF` cap transfer).
///
