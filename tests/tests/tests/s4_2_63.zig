/// §4.2.63 — `vm_intc_deassert_irq` returns `E_OK` on success.
...
pub fn main(_: u64) void {
    t.skipNoAarch64Vm("§4.2.63");
    const cr = syscall.vm_create(1, @intFromPtr(&policy));
    t.skipIfNoVm("§4.2.63", cr);
    if (cr < 0) {
    ...
    var irq: u64 = 0;
    while (irq < 24) : (irq += 1) {
        const result = syscall.vm_intc_deassert_irq(@bitCast(cr), irq);
