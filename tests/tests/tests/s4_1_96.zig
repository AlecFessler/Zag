...
/// §4.1.96 — `pmu_read` with invalid or wrong-type `thread_handle` returns `E_BADHANDLE`.
pub fn main(_: u64) void {
    ...
    if (rc_bad != syscall.E_BADHANDLE) {
        t.failWithVal("§4.1.96 bogus", syscall.E_BADHANDLE, rc_bad);
        syscall.shutdown();
    }
    ...
    if (rc_wrong != syscall.E_BADHANDLE) {
        t.failWithVal("§4.1.96 wrong-type", syscall.E_BADHANDLE, rc_wrong);
        syscall.shutdown();
    }
