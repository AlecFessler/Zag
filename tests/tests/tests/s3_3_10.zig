...
/// §3.3.10 — `recv` with blocking flag blocks when the queue is empty.
///
...
pub fn main(_: u64) void {
    ...
    if (@atomicLoad(u64, @as(*u64, @ptrCast(@volatileCast(&buf[2]))), .acquire) != 1) {
        t.fail("§3.3.10 recv returned before send");
        syscall.shutdown();
    }
    ...
    if (send_rc != 0) {
        t.failWithVal("§3.3.10 send", 0, send_rc);
        syscall.shutdown();
    }
