...
/// §4.1.71 — `fault_write_mem` with `buf_ptr` not readable in the caller's address space returns `E_BADADDR`.
///
...
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.71 proc_create", 1, child_ret);
        syscall.shutdown();
    }
