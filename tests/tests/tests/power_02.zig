// Spec §[power] power_reboot — test 02.
//
...
//   are explicitly NOT set on `child_self`) so a test can't reboot the
//   suite mid-run. That means the precondition for this E_PERM path is
//   already in place at test entry — no `restrict` setup is needed. We
//   only need to guard against a runner regression that would silently
//   grant `power`, which would let `power_reboot` actually fire and
//   tear the suite down.
//
// Action
//   1. read self-handle caps from slot 0 of the cap table; assert
//      `power` (bit 8) is clear so the runner contract still holds.
//   2. power_reboot() — must return E_PERM.
//
// Assertions
//   1: self-handle unexpectedly carries `power` (runner contract
//      broken; refusing to invoke the syscall is the only safe thing
//      to do, since success would actually reboot the system).
//   2: power_reboot returned something other than E_PERM.
...
pub fn main(cap_table_base: u64) void {
    ...
    const result = syscall.powerReboot();
