// Spec §[unmap_guest] unmap_guest — test 03.
//
// "[test 03] returns E_INVAL if N is 0."
//
// Strategy
//   To isolate the N == 0 rejection from every other gate in
//   §[unmap_guest] we need a real, valid VM handle so test 01
...
//   exactly like create_vcpu_02 / map_guest_05 / map_guest_03. With a
//   valid VM the only gate left for an empty page_frame list is
//   N == 0, which the spec requires to return E_INVAL.
//
...
//
//   The libz unmapGuest wrapper computes N as `page_frames.len`
//   directly (one page_frame handle per vreg, not paired), so an
...
//   3. unmapGuest(vm, &.{}) — must return E_INVAL.
//
// Assertions
...
//   5: unmap_guest with N == 0 returned a value other than E_INVAL
...
pub fn main(cap_table_base: u64) void {
    ...
    // 5. Empty page_frames slice → N == 0 in the syscall word. The
    //    libz wrapper computes `n = page_frames.len`, so the empty
    //    slice threads through to the kernel without a client-side
    //    guard.
    const result = syscall.unmapGuest(vm_handle, &.{});
