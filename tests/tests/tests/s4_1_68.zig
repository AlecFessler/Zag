...
/// §4.1.68 — `fault_write_mem` requires the `fault_handler` bit on `proc_handle`; returns `E_PERM` without it
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.68 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.68", E_PERM, ret);
