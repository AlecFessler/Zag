...
/// §2.4.9 — Virtual BAR access that begins at or extends beyond `port_count` kills the process with `invalid_read` or `invalid_write`.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    _ = t.requirePioDevice(view, "§2.4.9");
    const dev_handle = findPioDevice(view);
    if (dev_handle == 0) {
        t.fail("§2.4.9 no_pio_device");
        syscall.shutdown();
    }
    ...
    const child_ret = syscall.proc_create(
        @intFromPtr(children.child_vbar_oob_read.ptr),
        children.child_vbar_oob_read.len,
        child_rights,
    );
    if (child_ret < 0) {
        t.failWithVal("§2.4.9 proc_create", 0, child_ret);
        syscall.shutdown();
    }
    ...
    // Wait for the child to die — should be invalid_read or invalid_write.
    const reason = waitForDeath(view, child_handle);
    if (reason == .invalid_read or reason == .invalid_write) {
        t.pass("§2.4.9");
    } else {
        t.failWithVal("§2.4.9", @intFromEnum(perm_view.CrashReason.invalid_read), @intFromEnum(reason));
    }
    syscall.shutdown();
}
