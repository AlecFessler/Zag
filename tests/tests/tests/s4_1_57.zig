...
/// §4.1.57 — `fault_reply` returns `E_OK` on success
pub fn main(_: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.57 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (token_ret < 0) {
        t.failWithVal("§4.1.57 fault_recv", 0, token_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.57", E_OK, reply_ret);
