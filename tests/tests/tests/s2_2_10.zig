...
/// §2.2.10 — The initial thread's handle is inserted at slot 1 of the child process's permissions table during `proc_create`, with `ThreadHandleRights` as specified by the `thread_rights` parameter to `proc_create`
pub fn main(_: u64) void {
    ...
    if (ret != 0) {
        t.failWithVal("§2.2.10 ipc_call", 0, ret);
        syscall.shutdown();
    }
    ...
    if (entry_type != perm_view.ENTRY_TYPE_THREAD) {
        t.fail("§2.2.10 slot 1 entry_type not THREAD");
        pass_all = false;
    }
    if (handle == 0) {
        t.fail("§2.2.10 slot 1 handle is zero");
        pass_all = false;
    }
    if (rights != expected_rights) {
        t.fail("§2.2.10 slot 1 rights mismatch");
        pass_all = false;
    }
