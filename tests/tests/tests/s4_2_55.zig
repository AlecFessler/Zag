/// §4.2.55 — `vm_vcpu_interrupt` with `interrupt_ptr` not readable returns `E_BADADDR`.
...
pub fn main(pv: u64) void {
    t.skipNoAarch64Vm("§4.2.55");
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const self_handle: u64 = @bitCast(syscall.thread_self());

    const cr = syscall.vm_create(1, @intFromPtr(&policy));
    t.skipIfNoVm("§4.2.55", cr);
    if (cr < 0) {
        t.failWithVal("§4.2.55 create", syscall.E_OK, cr);
        syscall.shutdown();
    }

    const vcpu_handle = findVcpuHandle(view, self_handle);
    if (vcpu_handle == 0) {
        t.fail("§4.2.55 no vCPU handle");
    ...
    t.expectEqual("§4.2.55", syscall.E_BADADDR, result);
