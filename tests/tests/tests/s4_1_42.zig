...
/// §4.1.42 — The only way a process may hold `ThreadHandleRights.pmu` on another process's threads is to hold `fault_handler` for that process.
pub fn main(pv: u64) void {
    ...
    if (token < 0) {
        t.failWithVal("§4.1.42 fault_recv", 0, token);
        syscall.shutdown();
    }
    ...
    for (0..128) |i| {
        ...
        if ((e.rights & pmu_bit) == 0) {
            t.fail("§4.1.42 debuggee thread handle lacks pmu right");
            _ = syscall.fault_reply_simple(@bitCast(token), syscall.FAULT_KILL);
            syscall.shutdown();
        }
    }
    if (!found_any) {
        t.fail("§4.1.42 no debuggee thread handle found");
        _ = syscall.fault_reply_simple(@bitCast(token), syscall.FAULT_KILL);
        syscall.shutdown();
    }
