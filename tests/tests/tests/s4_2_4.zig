...
pub fn main(pv: u64) void {
    ...
    if (res.val < 0) {
        t.failWithVal("§4.2.4 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.2.4 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.2.4 set_state0", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    sr = syscall.vm_vcpu_set_state(vcpu1, @intFromPtr(&guest_state));
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.2.4 set_state1", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
