...
pub fn main(pv: u64) void {
    ...
    if (res.val < 0) {
        t.failWithVal("§4.2.10 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.2.10 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.2.10 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (exit_tag != EXIT_TAG_IO) {
        t.failWithVal("§4.2.10 exit_tag", EXIT_TAG_IO, @as(i64, exit_tag));
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (io_port == 0x42) {
        ...
        t.failWithVal("§4.2.10 port", 0x42, @as(i64, io_port));
    }
