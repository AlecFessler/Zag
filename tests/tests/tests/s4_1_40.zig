...
/// §4.1.40 — `ProcessRights.pmu` flows to child processes via the `process_rights` parameter of `proc_create` under the usual subset rule.
pub fn main(_: u64) void {
    ...
    if (reply_yes.words[0] != 1) {
        t.fail("§4.1.40 pmu did not flow into child");
        syscall.shutdown();
    }
    ...
    if (reply_no.words[0] != 0) {
        t.fail("§4.1.40 pmu leaked into subset-less child");
        syscall.shutdown();
    }
