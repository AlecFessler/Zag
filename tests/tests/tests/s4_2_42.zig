/// §4.2.42 — `vm_vcpu_set_state` returns `E_OK` on success.
...
pub fn main(pv: u64) void {
    t.skipNoAarch64Vm("§4.2.42");
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const self_handle: u64 = @bitCast(syscall.thread_self());

    const cr = syscall.vm_create(1, @intFromPtr(&policy));
    t.skipIfNoVm("§4.2.42", cr);
    if (cr < 0) {
        t.failWithVal("§4.2.42 create", syscall.E_OK, cr);
        syscall.shutdown();
    }

    const vcpu_handle = findVcpuHandle(view, self_handle);
    if (vcpu_handle == 0) {
        t.fail("§4.2.42 no vCPU handle");
    ...
    t.expectEqual("§4.2.42", syscall.E_OK, result);
