...
/// §2.2.28 — A `.faulted` thread is not scheduled and does not appear on any run queue.
///
...
pub fn main(_: u64) void {
    ...
    if (token1 < 0) {
        t.failWithVal("§2.2.28 fault_recv 1", 0, token1);
        syscall.shutdown();
    }
    ...
    if (rc != 0) {
        t.failWithVal("§2.2.28 fault_reply RESUME", 0, rc);
        syscall.shutdown();
    }
    ...
    if (token2 < 0) {
        t.failWithVal("§2.2.28 fault_recv 2", 0, token2);
        syscall.shutdown();
    }
    ...
    if (fm2.fault_addr == addr1) {
        ...
        t.fail("§2.2.28 second fault at different address");
    }
