...
/// §2.2.54 — `thread_resume` returns `E_OK` on success.
///
...
pub fn main(_: u64) void {
    ...
    if (thread_handle <= 0) {
        t.failWithVal("§2.2.54 thread_create", 1, thread_handle);
        syscall.shutdown();
    }
    ...
    if (suspend_ret != E_OK) {
        t.failWithVal("§2.2.54 suspend", E_OK, suspend_ret);
        syscall.shutdown();
    }
    ...
    if (@atomicLoad(u64, &counter, .seq_cst) != stalled) {
        t.fail("§2.2.54 counter advanced while suspended");
        syscall.shutdown();
    }

    const ret = syscall.thread_resume(@bitCast(thread_handle));
    t.expectEqual("§2.2.54 resume rc", E_OK, ret);

    // Wait for observable forward progress.
    var spins: u64 = 0;
    while (@atomicLoad(u64, &counter, .seq_cst) == stalled) : (spins += 1) {
        if (spins > 100000) {
            t.fail("§2.2.54 counter did not advance after resume");
            syscall.shutdown();
        }
        syscall.thread_yield();
    }
    t.pass("§2.2.54 worker advanced after resume");
    syscall.shutdown();
}
