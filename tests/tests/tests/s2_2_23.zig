...
/// §2.2.23 — `thread_kill` requires the `kill` right on the thread handle; returns `E_PERM` without it.
pub fn main(_: u64) void {
    ...
    if (ret != 0) {
        t.failWithVal("§2.2.23 ipc_call", 0, ret);
        syscall.shutdown();
    }
    const action_rc: i64 = @bitCast(reply.words[3]);
    t.expectEqual("§2.2.23", E_PERM, action_rc);
    syscall.shutdown();
}
