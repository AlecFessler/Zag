// Spec §[unmap_guest] unmap_guest — test 02.
//
// "[test 02] returns E_BADCAP if any [2 + i] is not a valid page_frame handle."
//
// Strategy
//   To isolate the page_frame BADCAP gate we need:
//     - test 01 (invalid VM)   — pass a freshly-minted VM handle.
//     - test 03 (N == 0)       — N = 1 (one page_frame entry).
//
//   With a real VM handle and N = 1, the only remaining gate that can
//   fire before the per-entry page_frame validation is the test-02 BADCAP
//   check on the supplied page_frame slot.
//
...
//   Userspace may then mint additional handles into the next free slots
//   (page_frame for the policy frame, VAR for the policy mapping, VM
//   itself). Slot 0xFFE — near the top of the 12-bit handle space — is
//   guaranteed empty by construction, so it is invalid as a page_frame
//   handle.
//
// VM setup mirrors map_guest_05 / unmap_guest_01 lead-up: stage a 4-KiB
// VmPolicy page frame, map it through a VAR so userspace can zero the
// policy bytes (so num_cpuid_responses / num_cr_policies are 0 — both
// well under MAX_*), then create_virtual_machine with caps={.policy=true}.
// The runner grants `crvm` and vm_ceiling = 0x01 (the policy bit), so caps
// stay subset of every ceiling. On a host without hardware virtualization
// the call returns E_NODEV — that path makes test 02 unreachable through
// any construction, so we smoke-pass (assertion id 0).
//
// Action
//   1. Stage VmPolicy (PF + VAR + map_pf + zero).
//   2. createVirtualMachine — VM handle (or smoke-pass on E_NODEV).
//   3. unmapGuest(vm, &.{ 0xFFE }) — must return E_BADCAP.
//
// Assertions
...
//   5: unmap_guest with an invalid page_frame handle returned a value
...
pub fn main(cap_table_base: u64) void {
    ...
    // 5. Slot 0xFFE is guaranteed empty by the create_capability_domain
    //    table layout — only slots 0..3 plus the few minted above are
    //    populated. The BADCAP gate on the page_frame entry must fire
    //    once the kernel discovers the empty slot.
    const invalid_pf: u64 = 0xFFE;
    const result = syscall.unmapGuest(vm_handle, &.{invalid_pf});
