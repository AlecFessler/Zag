...
pub fn main(cap_table_base: u64) void {
    ...
    // stack frame, so we reserve 920 bytes (word at offset 0, entry
    // at offset 912), syscall, then unwind. libz's replyTransfer
    // panics on N > 0; the open-coded path here is the documented
    // workaround.
