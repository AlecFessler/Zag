...
/// §3.1.4 — SHM handles are transferable if the `grant` bit is set.
pub fn main(perm_view: u64) void {
    ...
    if (reply.words[0] == 0xBEEF_CAFE) {
        ...
        t.failWithVal("§3.1.4", @bitCast(@as(u64, 0xBEEF_CAFE)), @bitCast(reply.words[0]));
    }
    syscall.shutdown();
}
