...
/// §4.1.64 — `fault_read_mem` with `vaddr` not mapped in the target's address space returns `E_BADADDR`
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.64 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.1.64 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }

    const proc_handle = findFaultHandlerProcHandle(view);
    if (proc_handle == 0) {
        t.fail("§4.1.64 no fault_handler proc handle");
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.64", E_BADADDR, ret);
