// Spec §[vm_set_policy] vm_set_policy — test 01.
//
...
//   as a VM handle.
//
//   The BADCAP gate on [1] must fire before any validation of kind,
//   count, or entries; we pass kind = 0, count = 0, and an empty
//   entries slice so no other validation can mask the BADCAP signal.
//
// Action
//   1. vm_set_policy(invalid_vm_slot, kind = 0, count = 0, &.{}) —
//      must return E_BADCAP because the VM slot is empty.
//
// Assertions
//   1: vm_set_policy returned something other than E_BADCAP.
...
pub fn main(cap_table_base: u64) void {
    ...
    // validation of kind/count/entries.
    const empty_slot: u12 = caps.HANDLE_TABLE_MAX - 1;

    const result = syscall.vmSetPolicy(empty_slot, 0, 0, &.{});
