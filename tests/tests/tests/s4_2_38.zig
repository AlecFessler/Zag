/// §4.2.38 — `vm_reply` with `action_ptr` not readable returns `E_BADADDR`.
...
pub fn main(pv: u64) void {
    ...
    t.skipIfNoVm("§4.2.38", cr);
    if (cr < 0) {
        t.failWithVal("§4.2.38 create", syscall.E_OK, cr);
        syscall.shutdown();
    }

    const vcpu_handle = findVcpuHandle(view, self_handle);
    if (vcpu_handle == 0) {
        t.fail("§4.2.38 no vCPU handle");
    ...
    if (exit_token <= 0) {
        t.failWithVal("§4.2.38 recv", 1, exit_token);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.2.38", syscall.E_BADADDR, result);
