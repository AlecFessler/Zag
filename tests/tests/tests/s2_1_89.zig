...
/// §2.1.89 — `proc_create` with `max_thread_priority` exceeding the parent's own `max_thread_priority` returns `E_PERM`.
///
...
pub fn main(_: u64) void {
    ...
    t.expectEqual("§2.1.89 child ceiling=NORMAL, grandchild ceiling=HIGH", E_PERM, r);
    ...
    if (r2 != E_PERM) {
        t.pass("§2.1.89 at-ceiling not E_PERM");
    } else {
        t.fail("§2.1.89 at-ceiling should not be E_PERM");
    }
