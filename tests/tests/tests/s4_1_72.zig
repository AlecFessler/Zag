...
/// §4.1.72 — `fault_write_mem` with `len` = 0 returns `E_INVAL`.
///
...
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.72 proc_create", 1, child_ret);
        syscall.shutdown();
    }
