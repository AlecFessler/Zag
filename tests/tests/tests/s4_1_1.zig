...
/// §4.1.1 — `ProcessRights` bit 7 is `fault_handler`.
///
...
pub fn main(pv: u64) void {
    ...
    if (pre_rc != E_AGAIN) {
        t.failWithVal("§4.1.1 pre fault_recv not empty", E_AGAIN, pre_rc);
        syscall.shutdown();
    }
    ...
    if (call_rc != 0) {
        t.failWithVal("§4.1.1 ipc_call", 0, call_rc);
        syscall.shutdown();
    }

    const child_token: i64 = @bitCast(reply.words[0]);
    if (child_token <= 0) {
        t.failWithVal("§4.1.1 child fault_recv token", 1, child_token);
        syscall.shutdown();
    }
    ...
    if (view[slot].entry_type != perm_view.ENTRY_TYPE_PROCESS) {
        t.fail("§4.1.1 child died unexpectedly");
        syscall.shutdown();
    }
    if (view[slot].processRestartCount() != 0) {
        t.fail("§4.1.1 child restarted (fault went to wrong box)");
        syscall.shutdown();
    }
    ...
    if (post_rc != E_AGAIN) {
        t.failWithVal("§4.1.1 fault leaked to parent", E_AGAIN, post_rc);
        syscall.shutdown();
    }
