...
/// §4.1.29 — `fault_set_thread_mode` with mode `stop_all` clears both `exclude_oneshot` and `exclude_permanent` on the thread's perm entry in the caller's permissions table.
pub fn main(pv: u64) void {
    ...
    if (thread_handle == 0) {
        t.fail("§4.1.29 no thread handle");
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, thread_handle)) |e| {
        if (!e.threadExcludePermanent()) {
            t.fail("§4.1.29 setup: exclude_permanent not visible");
            syscall.shutdown();
        }
    } else {
        t.fail("§4.1.29 thread entry vanished");
        syscall.shutdown();
    }

    const rc = syscall.fault_set_thread_mode(thread_handle, syscall.FAULT_MODE_STOP_ALL);
    if (rc != 0) {
        t.failWithVal("§4.1.29 stop_all rc", 0, rc);
        syscall.shutdown();
    }
    ...
    if (findThreadEntry(view, thread_handle)) |e| {
        if (!e.threadExcludeOneshot() and !e.threadExcludePermanent()) {
            ...
            t.fail("§4.1.29 B flags not cleared after stop_all");
        }
    } else {
        t.fail("§4.1.29 B thread entry vanished after stop_all");
    }
    syscall.shutdown();
}
