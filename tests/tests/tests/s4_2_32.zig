/// §4.2.32 — `vm_recv` returns the exit token (positive u64) on success.
...
pub fn main(pv: u64) void {
    t.skipNoAarch64Vm("§4.2.32");
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    const self_handle: u64 = @bitCast(syscall.thread_self());

    const cr = syscall.vm_create(1, @intFromPtr(&policy));
    t.skipIfNoVm("§4.2.32", cr);
    if (cr < 0) {
        t.failWithVal("§4.2.32 create", syscall.E_OK, cr);
        syscall.shutdown();
    }
    ...
    if (res.val < 0) {
        t.failWithVal("§4.2.32 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.2.32 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.2.32 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    // Blocking recv — should return positive exit token.
    const exit_token = syscall.vm_recv(@bitCast(cr), @intFromPtr(&buf), 1);
    if (exit_token > 0) {
        t.pass("§4.2.32");
    } else {
        t.failWithVal("§4.2.32", 1, exit_token);
    }
