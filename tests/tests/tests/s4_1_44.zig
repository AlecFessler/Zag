...
/// §4.1.44 — PMU state on a thread is created lazily.
///
...
pub fn main(_: u64) void {
    ...
    if (worker <= 0) {
        t.failWithVal("§4.1.44 thread_create", 1, worker);
        syscall.shutdown();
    }
    ...
    if (read_rc != syscall.E_INVAL) {
        t.failWithVal("§4.1.44 pmu_read lazy", syscall.E_INVAL, read_rc);
    ...
    if (stop_rc != syscall.E_INVAL) {
        t.failWithVal("§4.1.44 pmu_stop lazy", syscall.E_INVAL, stop_rc);
        _ = syscall.thread_kill(worker_h);
        syscall.shutdown();
    }
