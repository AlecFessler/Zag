...
/// §5.3.12 — On success with `cores_ptr` non-null, `sys_info` writes `SysInfo` to `info_ptr` and a fully populated `CoreInfo` array to `cores_ptr`, and resets each core's `idle_ns` and `busy_ns` atomically as they are read.
///
...
pub fn main(_: u64) void {
    ...
    while (i < info1.core_count) : (i += 1) {
        ...
        const still_sentinel =
            ...
            c.freq_hz == sentinel;
        if (still_sentinel) {
            t.failWithVal("§5.3.12 entry not populated", @intCast(i), @intCast(info1.core_count));
            syscall.shutdown();
        }
        total1 += c.idle_ns + c.busy_ns;
    }
    if (total1 == 0) {
        t.fail("§5.3.12 first sample: zero total accounting (cannot prove reset)");
        syscall.shutdown();
    }
    ...
    if (total2 >= total1) {
        t.failWithVal(
            "§5.3.12 second sample not smaller — reset did not happen",
            @intCast(total1),
            @intCast(total2),
        );
        syscall.shutdown();
