...
/// §4.1.74 — `fault_set_thread_mode` returns `E_OK` on success.
///
...
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.74 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.74 EXCLUDE_NEXT rc", E_OK, ret_next);
    if (!view[slot].threadExcludeOneshot()) {
        t.fail("§4.1.74 oneshot bit not set");
        syscall.shutdown();
    }
    ...
    t.expectEqual("§4.1.74 EXCLUDE_PERMANENT rc", E_OK, ret_perm);
    if (!view[slot].threadExcludePermanent()) {
        t.fail("§4.1.74 permanent bit not set");
        syscall.shutdown();
    }

    // STOP_ALL clears both.
    const ret_stop = syscall.fault_set_thread_mode(thread_handle, syscall.FAULT_MODE_STOP_ALL);
    t.expectEqual("§4.1.74 STOP_ALL rc", E_OK, ret_stop);
    if (view[slot].threadExcludeOneshot() or view[slot].threadExcludePermanent()) {
        t.fail("§4.1.74 exclude flags not cleared by STOP_ALL");
        syscall.shutdown();
    }
