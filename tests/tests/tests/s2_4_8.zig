...
/// §2.4.8 — Virtual BAR access with a non-MOV instruction kills the process with `protection_fault`.
pub fn main(pv: u64) void {
    const view: [*]const perm_view.UserViewEntry = @ptrFromInt(pv);

    _ = t.requirePioDevice(view, "§2.4.8");
    const dev_handle = findPioDevice(view);
    if (dev_handle == 0) {
        t.fail("§2.4.8 no_pio_device");
        syscall.shutdown();
    }
    ...
    if (child_ret < 0) {
        t.failWithVal("§2.4.8 proc_create", 0, child_ret);
        syscall.shutdown();
    }
    ...
    if (reason == .protection_fault) {
        ...
        t.failWithVal("§2.4.8", @intFromEnum(perm_view.CrashReason.protection_fault), @intFromEnum(reason));
    }
    syscall.shutdown();
}
