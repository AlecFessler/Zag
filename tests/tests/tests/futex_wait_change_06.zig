// Spec §[futex_wait_change] futex_wait_change — test 06.
//
// "[test 06] returns E_TIMEOUT if the timeout expires before any pair's
//  `addr == target` condition is met and before any watched address is
//  woken."
//
// Strategy
//   futex_wait_change blocks while every `(addr, target)` pair satisfies
//   `*addr != target`. To exercise the timeout path we have to:
//     1. Hand the syscall a single watched address whose current value
//        is unequal to its target (so the call must actually block —
//        the entry-time fast path of test 07 does not fire).
...
//        non-blocking poll, and u64::MAX is "indefinite"; neither would
...
//
//   We seed the watched word with value 0 and pass target = 1, so
//   `*addr == target` is false at entry and stays false for the whole
...
//   enough not to materially extend the suite runtime if many wait-
//   change tests trigger this path.
//
//   N = 1 keeps the call within the register-passed argument fast path
//   (pairs.len = 2 fits in v2/v3 of `futexWaitChange`), and stays well
...
//   2. futex_wait_change(timeout_ns = 10_000_000,
//                        pairs = { &watched, target = 1 }).
//   3. Assert vreg 1 == E_TIMEOUT.
//
// Assertions
//   1: futex_wait_change did not return E_TIMEOUT (vreg 1 mismatch).
...
pub fn main(cap_table_base: u64) void {
    ...
    // target != *watched so the entry-time fast path (test 07) does not
    // fire and the call has to block.
    const pairs = [_]u64{ @intFromPtr(&watched), 1 };
