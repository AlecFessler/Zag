// Spec §[idc_write] idc_write — test 03.
//
...
//   idc_write, every other precondition must be satisfied:
//     - [1] must be a valid VAR handle with the `w` cap (defeats
//       tests 01 and 02),
...
//     2. createVar(caps={r,w}, props=0, pages=1) — VAR.caps has `w`
//        (defeats test 02), sz=0 (4 KiB), pages=1 → size = 4096 bytes.
//     3. mapPf(var, &.{ 0, pf }) — install the page_frame so the VAR
//        has live backing for the write path the kernel may exercise
...
//
//   Action:
//     idcWrite(var, offset = 1, qwords = &.{ 1 }). offset = 1 is not a
//     multiple of 8, so the alignment gate must fire E_INVAL per
//     §[idc_write] test 03. count = 1 keeps test 04 inactive; with
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 2: VAR with caps = r|w (the `w` cap defeats §[idc_write]
    // test 02). props = 0 → cur_rwx = 0, sz = 0, cch = 0. pages = 1
    // gives a 4 KiB VAR, large enough that offset = 1, count = 1
    // (byte range 1..9) does not trip §[idc_write] test 05.
    ...
    // Step 4: idc_write with offset = 1 (not 8-byte aligned), count = 1.
    // Per §[idc_write] test 03, the kernel must return E_INVAL.
    const result = syscall.idcWrite(var_handle, 1, &.{1});
