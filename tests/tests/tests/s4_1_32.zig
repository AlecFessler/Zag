...
/// §4.1.32 — `fault_set_thread_mode` requires that the calling process holds `fault_handler` for the owning process of the target thread (the thread handle appears in the caller's perm table as a thread-type entry belonging to a process whose `fault_handler_proc == caller`).
...
/// `fault_handler` for itself, §4.1.32 must reject with E_PERM. The
...
pub fn main(_: u64) void {
    ...
    // kernel cannot short-circuit to E_BADCAP — so §4.1.32 must return
    // E_PERM exclusively.
    ...
    if (syscall.ipc_call(probe_handle, &.{0}, &reply) != 0) {
        t.fail("§4.1.32 B ipc_call");
        syscall.shutdown();
    }

    const child_rc: i64 = @bitCast(reply.words[0]);
    if (child_rc != E_PERM) {
        t.failWithVal("§4.1.32 B expected E_PERM", E_PERM, child_rc);
        syscall.shutdown();
    }
