...
/// §4.1.70 — `fault_write_mem` with `vaddr` not mapped in the target's address space returns `E_BADADDR`
pub fn main(pv: u64) void {
    ...
    if (child_ret <= 0) {
        t.failWithVal("§4.1.70 proc_create", 1, child_ret);
        syscall.shutdown();
    }
    ...
    if (recv_ret < 0) {
        t.failWithVal("§4.1.70 fault_recv", 0, recv_ret);
        syscall.shutdown();
    }

    const proc_handle = findFaultHandlerProcHandle(view);
    if (proc_handle == 0) {
        t.fail("§4.1.70 no fault_handler proc handle");
        syscall.shutdown();
    }

    var buf: [8]u8 = .{0} ** 8;
    const ret = syscall.fault_write_mem(proc_handle, 0xDEAD_0000, @intFromPtr(&buf), 8);
    t.expectEqual("§4.1.70", E_BADADDR, ret);
