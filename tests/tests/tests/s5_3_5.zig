...
/// §5.3.5 — `CoreInfo.freq_hz` is the current CPU frequency of the core in hertz.
///
...
pub fn main(_: u64) void {
    ...
    while (i < info.core_count) : (i += 1) {
        ...
        if (hz < MIN_PLAUSIBLE_HZ) {
            t.failWithVal(
                "§5.3.5 freq_hz too low (unit bug?)",
    ...
    if (!any_nonzero) {
        t.pass("§5.3.5 (all cores reported 0 Hz — stub / unvirtualised MSR)");
    } else {
        t.pass("§5.3.5");
    }
    syscall.shutdown();
