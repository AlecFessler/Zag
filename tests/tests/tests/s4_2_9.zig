...
pub fn main(pv: u64) void {
    ...
    if (res.val < 0) {
        t.failWithVal("§4.2.9 reserve", 0, res.val);
    ...
    if (mr != syscall.E_OK) {
        t.failWithVal("§4.2.9 vm_guest_map", syscall.E_OK, mr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (sr != syscall.E_OK) {
        t.failWithVal("§4.2.9 set_state", syscall.E_OK, sr);
        _ = syscall.revoke_vm(@bitCast(cr));
        syscall.shutdown();
    }
    ...
    if (guest_rip >= 4) {
        ...
        t.failWithVal("§4.2.9 rip", 4, @as(i64, @bitCast(guest_rip)));
    }
