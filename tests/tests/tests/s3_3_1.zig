...
/// §3.3.1 — `send` is non-blocking: the sender continues running after delivery.
///
...
pub fn main(_: u64) void {
    ...
    if (rc != 0) {
        ...
        t.failWithVal("§3.3.1 send failed", 0, rc);
        syscall.shutdown();
    }
    ...
    if (!saw) {
        @atomicStore(u64, @as(*u64, @ptrCast(@volatileCast(&buf[7]))), 0, .release);
        _ = syscall.futex_wake(@as(*u64, @ptrCast(@volatileCast(&buf[7]))), 1);
        t.fail("§3.3.1 recorder never saw msg");
        syscall.shutdown();
    }
